...
public:
	// 调用MCP工具(同步,带超时)
	// mcpName: MCP工具名称(即LlmToolCall::mcpName)
	// arguments: 工具参数的JSON字符串(即LlmToolCall::raw_arguments)
	// result: 输出,工具调用的结果
	// hCancel: 取消事件句柄(可选,置位时立即返回false)
	// 返回: 成功与否
	bool CallTool(const std::string& mcpName, const std::string& arguments, std::string& result, HANDLE hCancel = nullptr, int timeoutMs = 30000);

private:
	// 根据工具名称查找所属的server(仅返回Ready状态的server)
	Server* _FindServerByToolName(const std::string& toolName);

	// 请求ID计数器（每次请求递增，确保唯一）
	std::atomic<long long> _nextRequestId{ 1 };
