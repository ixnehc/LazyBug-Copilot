...
void CChatTaskMgr::AddTask_AddFileToProject(const LlmToolCall& toolCall)
{
	CChatTask_AddFileToProject* task = new CChatTask_AddFileToProject;
	task->SetToolCall(toolCall);
	_AddTask(task);
}
...
void CChatTaskMgr::AddTask_CompressSummarize(int workingOpIndex, const std::string& summarizeApiName, CompressSummarizeMode mode)
{
	CChatTask_CompressSummarize* task = new CChatTask_CompressSummarize(workingOpIndex, summarizeApiName, mode);
	_AddTask(task);
}
...
void CChatTaskMgr::UpdateToolCalls(std::vector<LlmToolCall>& toolCalls)
{
	for (int k = 0;k < toolCalls.size();k++)
	{
		...
		case LlmToolType::AddFileToProject:
		{
			if (toolCall.IsComplete())
				AddTask_AddFileToProject(toolCall);
			break;
		}
