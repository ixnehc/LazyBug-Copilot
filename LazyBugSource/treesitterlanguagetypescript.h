...
{
	// TypeScript 语言支持
	class CLanguageSupportTypeScript : public ILanguageSupport
	{
	public:
		CLanguageSupportTypeScript();
		virtual ~CLanguageSupportTypeScript();

		virtual Language GetLanguage() const override { return Language::TypeScript; }
	...
	private:
		...
		bool IsTypeAliasDeclaration(TSNode node) const;
		bool IsEnumDeclaration(TSNode node) const;
		bool IsAbstractClassDeclaration(TSNode node) const;
		bool IsModuleDeclaration(TSNode node) const;
		bool IsAbstractMethodDefinition(TSNode node) const;
