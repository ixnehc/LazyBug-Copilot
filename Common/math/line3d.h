...
class line3d
{
	public:
		...
		void getProjectionPoint(const vector3d<T>& point,vector3d<T>&pointProj) const
		{
			vector3d<T> c = point - start;
			vector3d<T> v = end - start;
			T d = (T)v.getLength();
...
//		
//		bool getIntersectionWithSphere(sphere<T> sph, f64& outdistance)
//		{
//			vector3d<T> q = sph.center - start;
//			f64 c = q.getLength();
//			f64 v = q.dotProduct(getVector().normalize());
//			f64 d = sph.radius * sph.radius - (c*c - v*v);
//
//            if (d < 0.0) 
//				return false;
//
