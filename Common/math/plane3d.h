...
namespace i_math
{
	...
	public:
		...
		plane3d(T nx, T ny, T nz,T d) : Normal(nx, ny, nz),D(d) { };
		plane3d(const vector3d<T>& normal,T d) : Normal(normal),D(d) { };
		...
		inline plane3d<T>& operator+=(const vector3d<T>& point)
		{
		...
		//! \param lineVect: Vector of the line to intersect with,不需要(但最好是)normalized
		...
		//! \return Returns true if there was an intersection, false if there was not.
		bool clipLine(const vector3d<T>& linePoint, const vector3d<T>& lineVect,	
									T& tIntersection,bool &bClipForward) const
		{
			T t2 = Normal.dotProduct(lineVect);
			if ((t2 >= (T)-0.001f)&&(t2 <= (T)0.001f))
				return false;//Almost parallel
			tIntersection=- (Normal.dotProduct(linePoint) - D) / t2;	
			bClipForward=(t2>0);
		...
		//get the equation coefficient(ABCD) of this plane
		//note that the inequation of A*x+B*y+C*z+D>=0 represent the front of the plane
		void getEquation(vector4d<T>&abcd) const
		{
			vector3d<T>mp=getMemberPoint();
			abcd.x=Normal.x;
			abcd.y=Normal.y;
			abcd.z=Normal.z;
		...
		//! Returns the distance to a point.  Note that this only
		//! works if the normal is Normalized.
		void getProjectionOf(const vector3d<T>& point,vector3d<T>&pointProj) const
		{
			T dist;
			dist=point.dotProduct(Normal) -D;
			//dist=point.dotProduct(Normal) + D;
		...
		void flipVer(float y)
		{
			i_math::vector3df mpoint=getMemberPoint();
			mpoint.y=2*y-mpoint.y;
			Normal.y=-Normal.y;
			recalculateD(mpoint);
		}


		//bool operator==( const plane3d& V ) const
		//{
		//	return Normal==V.Normal && D==V.D;
		//}
		//bool operator!=( const plane3d& V ) const
		//{
		//	return Normal!=V.Normal|| D!=V.D;
		//}
		/*__forceinline T dotProduct( const plane3d& V ) const
		{
			return Normal.dotProduct(V.Normal) -D;
		}*/

		__forceinline T dotProduct( const vector3d<T>& MPoint ) const
		{
			return Normal.dotProduct(MPoint) -D;
		}
		plane3d operator+( const plane3d& V ) const
		{
			return plane3d(Normal+V .Normal , D + V.D );
		}
		plane3d operator-( const plane3d& V ) const
		{
			return plane3d(Normal-V .Normal , D - V.D );
		}
		plane3d operator/(const T  Scale ) const
		{
			T  RScale = 1.f/Scale;
			return plane3d(  Normal* RScale, D * RScale );
		}
		plane3d operator*( const T  Scale ) const
		{
			return plane3d(  Normal* Scale, D * Scale );
		}
		...
		plane3d operator*=( const T Scale )
		{
