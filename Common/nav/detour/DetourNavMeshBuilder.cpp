...
static int createBVTree(dtBoundBox *polysBox, const int npolys, dtBVNode* nodes)
{
	// Build tree
	...
	for(int i = 0;i<npolys;++i)
	{
		...
		it.bmin[0] = polysBox[i].bmin[0];
		it.bmin[1] = polysBox[i].bmin[1];
		it.bmin[2] = polysBox[i].bmin[2];
		
		it.bmax[0] = polysBox[i].bmax[0];
		it.bmax[1] = polysBox[i].bmax[1];
		it.bmax[2] = polysBox[i].bmax[2];
	}
...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	...
	for (int i = 0; i < params->vertCount; ++i)
	{
		...
		v[0] = dtClamp(v[0],params->tileBoundMin[0],params->tileBoundMax[0]);
		v[2] = dtClamp(v[2],params->tileBoundMin[1],params->tileBoundMax[1]);
	}
	...
	if (params->detailMeshes)
	{
		...
		for (int i = 0; i < params->polyCount; ++i)
		{
			...
			{	
				int n = ndv-nv;
				float *v = navDVerts + vbase*3;

				memcpy(v, &params->detailVerts[(vb+nv)*3], sizeof(float)*3*n);
	...
	// Calc Polygon bound
	dtBoundBox * polysBox = new dtBoundBox[params->polyCount];	
	float bmin[3],bmax[3];
	for(int i = 0;i<params->polyCount;++i)
	{
		dtPoly		  * poly = navPolys + i;
		dtPolyDetail  * polyDetail = navDMeshes + i;
		dtBoundBox    * bb = polysBox + i;

		int p = poly->verts[0];
		float *v = navVerts + p*3;
		dtVcopy(bmin,v);
		dtVcopy(bmax,v);
		...
		{
			const unsigned char * r = navDTris + (polyDetail->triBase + k)*4;
			for(int t = 0;t<3;++t)
			{
				if(r[t]<poly->vertCount)
					v = navVerts + poly->verts[r[t]]*3;
				else
					v = navDVerts + (polyDetail->vertBase + (r[t]-poly->vertCount))*3;
		...
		bmin[0] = dtClamp(bmin[0],params->bmin[0],params->bmax[0]) - params->bmin[0];
		bmin[1] = dtClamp(bmin[1],params->bmin[1],params->bmax[1]) - params->bmin[1];
		bmin[2] = dtClamp(bmin[2],params->bmin[2],params->bmax[2]) - params->bmin[2];

		bmax[0] = dtClamp(bmax[0],params->bmin[0],params->bmax[0]) - params->bmin[0];
		bmax[1] = dtClamp(bmax[1],params->bmin[1],params->bmax[1]) - params->bmin[1];
		bmax[2] = dtClamp(bmax[2],params->bmin[2],params->bmax[2]) - params->bmin[2];

		bb->bmin[0] = (unsigned short)floorf(bmin[0]/params->cs);
		bb->bmin[1] = (unsigned short)floorf(bmin[1]/params->cs);
		bb->bmin[2] = (unsigned short)floorf(bmin[2]/params->cs);
		
		bb->bmax[0] = (unsigned short)ceilf(bmax[0]/params->cs);
		bb->bmax[1] = (unsigned short)ceilf(bmax[1]/params->cs);
		bb->bmax[2] = (unsigned short)ceilf(bmax[2]/params->cs);
	}

	// Store and create BVtree.
	// TODO: take detail mesh into account! use byte per bbox extent?
	if (params->buildBvTree)
		createBVTree(polysBox,params->polyCount,navBvtree);
