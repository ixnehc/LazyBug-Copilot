...
bool dtQueryFilter::passFilter(const dtPolyRef /*ref*/,
									  ...
									  const dtPoly* poly) const
{
	if (m_tp==FindWalkable)
	{
		BYTE area=poly->getArea();
 		if ((area==63)||(area==2)||(area==3))
			return true;
	}
	else
	{
		if (m_tp==FindAdvWalkable)
		{
			BYTE area=poly->getArea();
			if ((area==1)||(area==63)||(area==2)||(area==3))
				return true;
		}

		if (m_tp==FindSwitchable)
		{
			BYTE area=poly->getArea();
			if ((area==2)||(area==3)||(area==4))
				return true;
		}
		if (m_tp==FindWalkableAndSwitchable)
		{
			BYTE area=poly->getArea();
			if ((area==2)||(area==3)||(area==4)||(area==63))
				return true;
		}
	}
	return  false; 
...
dtStatus dtNavMeshQuery::init(dtMeshBase* nav, int maxNodes)
{
	m_nav = nav;
										 ...
										 dtPolyRef* nearestRef, float* nearestPt) const
{
	...
	dtPolyRef nearest = DT_INVALID_POLYREF;
...
dtStatus dtNavMeshQuery::findPath(dtPolyRef startRef, dtPolyRef endRef,
								  ...
								  const dtQueryFilter* filter,
								  dtPolyRef* path, int* pathCount, const int maxPath,bool ignore_ext) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	dtAssert(m_openList);
	...
	*pathCount = 0;
	...
	if (startRef==DT_INVALID_POLYREF||endRef==DT_INVALID_POLYREF)
		return DT_FAILURE | DT_INVALID_PARAM;
	...
	while (!m_openList->empty())
	{
		...
		dtLinkAlloc * linkAlloc = bestTile->m_linkAlloc;
		assert(linkAlloc);

		// Get parent poly and tile.
		dtPolyRef parentRef = DT_INVALID_POLYREF;
		const dtMeshTile* parentTile = 0;
		const dtPoly* parentPoly = 0;
		if (bestNode->pidx)
			parentRef = m_nodePool->getNodeAtIdx(bestNode->pidx)->id;
		if (parentRef!=DT_INVALID_POLYREF)
			m_nav->getTileAndPolyByRefUnsafe(parentRef, &parentTile, &parentPoly);
		
		dtLink * link = 0;
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = link->next)
		{
			link = linkAlloc->getLink(i);

			dtPolyRef neighbourRef =  link->ref;
			
			// Skip invalid ids and do not expand back to where we came from.
			if (neighbourRef==DT_INVALID_POLYREF|| neighbourRef == parentRef)
				continue;
			
			// if ignore flag set, skip ext link.
			if(ignore_ext)
			{
				int it = m_nav->decodePolyIdTile(neighbourRef);
				if(link->side!= 0xff/*&&it!=itStart*/)
					continue;
			}
...
// 				if (dtVdistSqr(left,endPos)<dtVdistSqr(right,endPos))
// 					dtVcopy(neighbourNode->pos,left);
// 				else
// 					dtVcopy(neighbourNode->pos,right);
		...
		for (int i = 0; i < pathSize; ++i)
		{
			...
			if (dtTriArea2D(portalApex, portalRight, right) <= 0.0f)
			{
				if (dtVequal(portalApex, portalRight) || dtTriArea2D(portalApex, portalLeft, right) > 0.0f)
				{
					...
					rightPolyRef = (i+1 < pathSize) ? path[i+1] : DT_INVALID_POLYREF;
				...
				else
				{
					...
					if (leftPolyRef==DT_INVALID_POLYREF)
...
// 					if(apexIndex>i)
			...
			if ( dtTriArea2D(portalApex, portalLeft, left)>= 0.0f)
			{
				if (dtVequal(portalApex, portalLeft) || dtTriArea2D(portalApex, portalRight, left) < 0.0f)
				{
					...
					leftPolyRef = (i+1 < pathSize) ? path[i+1] : DT_INVALID_POLYREF;
					leftPolyType = toType;
					leftIndex = i;
				}
				else
				{
					...
					unsigned char flags = 0;
					if (rightPolyRef==DT_INVALID_POLYREF)
						flags = DT_STRAIGHTPATH_END;
...
// 					if(apexIndex>i)
	...
	if (n < maxStraightPath)
	{
		...
		if (straightPathRefs)
			straightPathRefs[n] = DT_INVALID_POLYREF;
		n++;
	}
										  ...
										  float* resultPos, dtPolyRef* visited, int* visitedCount, const int maxVisitedSize) const
{
	...
	while (nstack)
	{
		...
		dtLinkAlloc * linkAlloc = curTile->m_linkAlloc;
		assert(linkAlloc);
		...
		for (int i = 0, j = (int)curPoly->vertCount-1; i < (int)curPoly->vertCount; j = i++)
		{
			...
			if (curPoly->neis[j] & DT_EXT_LINK)
			{
				...
				{
					const dtLink* link =  linkAlloc->getLink(k);	// &curTile->links[k];
					if (link->edge == j)
					{
						if (link->ref != 0)
						{
										 ...
										 float* left, float* right) const
{
	...
	dtLinkAlloc * linkAlloc = fromTile->m_linkAlloc;
	assert(linkAlloc);
...
// 		}
		...
		dtLink *linkCur = linkAlloc->getLink(i);
		if(linkCur->ref==to)
		{
			link = linkCur;
		}
	...
	if (fromPoly->getType() == DT_POLYTYPE_OFFMESH_CONNECTION)
	{
		...
		for (unsigned int i = fromPoly->firstLink; i != DT_NULL_LINK; /*i = fromTile->links[i].next*/)
		{
			...
			if (/*fromTile->links[i]*/linkCur->ref == to)
			{
				const int v =	linkCur->edge;	//fromTile->links[i].edge;
	...
	if (toPoly->getType() == DT_POLYTYPE_OFFMESH_CONNECTION)
	{
		for (unsigned int i = toPoly->firstLink; i != DT_NULL_LINK; /*i = toTile->links[i].next*/)
		{
			...
			if (/*toTile->links[i]*/linkCur->ref == from)
			{
				const int v = linkCur->edge;//toTile->links[i].edge;
								 ...
								 float* t, float* hitNormal, dtPolyRef* path, int* pathCount, const int maxPath) const
{
	...
	*t = 0;
	...
	// Validate input
	if (startRef==DT_INVALID_POLYREF || !m_nav->isValidPolyRef(startRef))
		return DT_FAILURE | DT_INVALID_PARAM;
	...
	while (curRef)
	{
		...
		dtLinkAlloc * linkAlloc = tile->m_linkAlloc;
		assert(linkAlloc);

		const dtLink* link = 0;
		for (unsigned int i = poly->firstLink; i != DT_NULL_LINK; i = link->next)
		{
			link = linkAlloc->getLink(i); //&tile->links[i];
											   ...
											   int* resultCount, const int maxResult) const
{
	...
	*resultCount = 0;
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = link->next)
		{
			link = linkAlloc->getLink(i);//&bestTile->links[i];
												...
												int* resultCount, const int maxResult) const
{
	...
	*resultCount = 0;
	...
	while (nstack)
	{
		...
		const dtLink* link = 0;
		for (unsigned int i = curPoly->firstLink; i != DT_NULL_LINK; i = link->next)
		{
			link = linkAlloc->getLink(i);	//&curTile->links[i];
			dtPolyRef neighbourRef = link->ref;
			...
			for (int j = 0; j < n; ++j)
			{
				...
				for (unsigned int k = curPoly->firstLink; k != DT_NULL_LINK; /*k = curTile->links[k].next*/)
				{
					dtLink * curLink = linkAlloc->getLink(k);

					if (/*curTile->links[k]*/curLink->ref == pastRef)
					{
						connected = true;
						break;
					}
	...
	*segmentCount = 0;
	...
	for (int i = 0, j = (int)poly->vertCount-1; i < (int)poly->vertCount; j = i++)
	{
		...
		if (poly->neis[j] & DT_EXT_LINK)
		{
			...
			{
				const dtLink* link = linkAlloc->getLink(k);	//&tile->links[k];
				if (link->edge == j)
				{
					if (link->ref != 0)
					{
	...
	while (!m_openList->empty())
	{
		...
		for (int i = 0, j = (int)bestPoly->vertCount-1; i < (int)bestPoly->vertCount; j = i++)
		{
			...
			if (bestPoly->neis[j] & DT_EXT_LINK)
			{
				...
				{
					const dtLink* link = linkAlloc->getLink(k);		//&bestTile->links[k];
					if (link->edge == j)
					{
						if (link->ref != 0)
						{
...
void dtNavMeshQuery::switchWalkable(dtPolyRef *refs,int count, bool isOn)
{
	const dtMeshTile* tile = 0;
	const dtPoly* poly = 0;
	for (int i=0;i<count;i++)
	{
		if (dtStatusSucceed(m_nav->getTileAndPolyByRef(refs[i], &tile, &poly)))
		{
			dtPoly*poly2=(dtPoly*)poly;
			if (isOn)
				poly2->setArea(3);
			else
				poly2->setArea(4);
