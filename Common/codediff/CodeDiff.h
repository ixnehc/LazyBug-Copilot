...
extern int ClosestNewLineFromOldLine(int oldLine, const std::deque<CodeDiffLine>& diffs);
...
struct CodeComparingChars
{
	...
	std::wstring content;//比较内容,用于显示比较结果的文本文件
...
extern void MakeCodeComparing_Chars2(const std::string& oldCode, const std::string& newCode, CodeComparingChars& comparingContent);

extern std::string TrimCodeLineForCompare(const std::string& str);
