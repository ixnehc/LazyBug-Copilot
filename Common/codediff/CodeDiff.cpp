...
// Wide char版本的TrimCodeLineForCompare函数
std::wstring TrimCodeLineForCompareW(const std::wstring& str)
{
...

	{
		...
		for (const auto& diff : diffs)
		{
			if (diff.oldLineNumber > 0)
				oldLineCount = max(oldLineCount, diff.oldLineNumber);
			if (diff.newLineNumber > 0)
				newLineCount = max(newLineCount, diff.newLineNumber);
		}

		// ��ʼ���к�ӳ��
		comparingContent.oldLineIndices.resize(oldLineCount, -1);
		comparingContent.newLineIndices.resize(newLineCount, -1);
	}
...
void MakeCodeComparing_Chars(const std::string& oldCode, const std::string& newCode, CodeComparingChars& comparingContent)
{
    // 清空比较内容
    comparingContent.content.clear();
    ...
    MyersDiff<std::wstring> myers(wOldCode, wNewCode);
...
int ClosestNewLineFromOldLine(int oldLine, const std::deque<CodeDiffLine>& diffs)
{
	int newLine = -1;
	int minDiff = 1000000;
	for (int i = 0; i < diffs.size(); i++)
	{
		const CodeDiffLine& diff = diffs[i];
		if (diff.type == CodeDiffType::UNCHANGED)
		{
			if (abs(diff.oldLineNumber - oldLine) < minDiff)
			{
				if (diff.newLineNumber == 0)
				{
					if (newLine < 0)
					...
					newLine = diff.newLineNumber;
				minDiff = abs(diff.oldLineNumber - oldLine);
			}
		}
	}
	return newLine;
