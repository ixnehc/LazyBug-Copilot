...
class MyersDiff {
   ...
   public:
    ...
    struct Diff {
        DiffOp operation;
        Range text;
        Diff(DiffOp op, Range text_range)
   ...
   public:
    ...
    Diffs diff_main(Range text1, Range text2) {
        ...
        Time deadline;
        if (Diff_Timeout <= 0) {
			deadline = chrono::steady_clock::now() + chrono::milliseconds(0x7fffffff);
		}
		else {
    ...
    Diffs diff_main(Range text1, Range text2, Time deadline) {
        ...
        if (text1 == text2) {
            if (text1.size() != 0) {
                diffs.push_back(Diff(DiffOp_Equal, text1));
            }
            return diffs;
        }
        ...
        if (commonprefix.size() != 0) {
            diffs.insert(diffs.begin(), Diff(DiffOp_Equal, commonprefix));
        }
        if (commonsuffix.size() != 0) {
            diffs.push_back(Diff(DiffOp_Equal, commonsuffix));
        }
    ...
    Diffs diff_compute(Range text1, Range text2, Time deadline) {
        ...
        if (text1.size() == 0) {
            ...
            diffs.push_back(Diff(DiffOp_Insert, text2));
            return diffs;
        }

        if (text2.size() == 0) {
            // Just delete some text (speedup).
            diffs.push_back(Diff(DiffOp_Delete, text1));
            return diffs;
        }

        Range longtext = text1.size() > text2.size() ? text1 : text2;
        Range shorttext = text1.size() > text2.size() ? text2 : text1;
        int i = (int)longtext.find(shorttext);
        if (i != -1) {
            // Shorter text is inside the longer text (speedup).
            DiffOp op = (text1.size() > text2.size()) ? DiffOp_Delete : DiffOp_Insert;
            diffs.push_back(Diff(op, longtext.substr(0, i)));
            diffs.push_back(Diff(DiffOp_Equal, shorttext));
        ...
        if (hm != null) {
          ...
          // Merge the results.
          diffs = diffs_a;
          diffs.push_back(Diff(DiffOp_Equal, mid_common));
    ...
    Diffs diff_bisect(Range text1, Range text2, Time deadline) {
        ...
        int text1_length = (int)text1.size();
        int text2_length = (int)text2.size();
    ...
    int diff_commonPrefix(Range text1, Range text2) {
        ...
        int n = (int)min(text1.size(), text2.size());
        for (int i = 0; i < n; i++) {
            if (text1[i] != text2[i]) {
                return i;
            }
   ...
   */
    ...
    */
    ...
    diffs.listIterator();
      ...
      while (thisDiff != null) {
        if (thisDiff.operation == DiffOp_Equal) {
          // Equality found.
          ...
          if (thisDiff.operation == DiffOp_Insert) {
          ...
          if (lastEquality != null && (lastEquality.size()
            ...
            pointer.set(Diff(DiffOp_Delete, lastEquality));
            // Insert a corresponding an insert.
            pointer.push_back(Diff(DiffOp_Insert, lastEquality));
      ...
      while (thisDiff != null) {
        if (prevDiff.operation == DiffOp_Delete &&
            thisDiff.operation == DiffOp_Insert) {
    ...
    edits. pointer.previous(); pointer.push_back(Diff(DiffOp_Equal,
            ...
            if (overlap_length2 >= deletion.size() / 2.0 ||
              ...
              pointer.previous();
              pointer.push_back(Diff(DiffOp_Equal,
                                   deletion.substring(0, overlap_length2)));
              prevDiff.operation = DiffOp_Insert;
              prevDiff.text =
                insertion.substring(0, insertion.size() - overlap_length2);
              thisDiff.operation = DiffOp_Delete;
    ...
    */
    ...
    void diff_cleanupSemanticLossless(Diffs diffs) {
      ...
      while (nextDiff != null) {
        if (prevDiff.operation == DiffOp_Equal &&
            nextDiff.operation == DiffOp_Equal) {
    ...
    */
    ...
    unsplittable. while (thisDiff != null) { if (thisDiff.operation == DiffOp_Equal) {
          // Equality found.
          if (thisDiff.text.size() < Diff_EditCost && (post_ins || post_del))
    {
          ...
          // An insertion or deletion.
          if (thisDiff.operation == DiffOp_Delete) {
            post_del = true;
          } else {
            post_ins = true;
          }
          ...
          if (lastEquality != null
            ...
            pointer.set(Diff(DiffOp_Delete, lastEquality));
            // Insert a corresponding an insert.
            pointer.push_back(thisDiff = Diff(DiffOp_Insert, lastEquality));
    ...
    */
    ...
    void diff_cleanupMerge(Diffs diffs) {
      diffs.push_back(Diff(DiffOp_Equal, ""));  // Add a dummy entry at the end.
      std::vectorIterator<Diff> pointer = diffs.listIterator();
      int count_delete = 0;
      int count_insert = 0;
      Range text_delete = "";
        ...
        case DiffOp_Equal:
          if (count_delete + count_insert > 1) {
            ...
            if (both_types) {
              ...
              if (commonlength != 0) {
                if (pointer.hasPrevious()) {
                  thisDiff = pointer.previous();
                  assert thisDiff.operation == DiffOp_Equal
            ...
            if (text_delete.size() != 0) {
              pointer.push_back(Diff(DiffOp_Delete, text_delete));
            }
            if (text_insert.size() != 0) {
              pointer.push_back(Diff(DiffOp_Insert, text_insert));
            }
    ...
    */
    ...
    String diff_text1(Diffs diffs) {
        ...
        for (Diff aDiff : diffs) {
            if (aDiff.operation != DiffOp_Insert) {
    ...
    String diff_text2(Diffs diffs) {
        Range text{};
        for (Diff aDiff : diffs) {
            if (aDiff.operation != DiffOp_Delete) {
                text.append(aDiff.text);
            }
        }
        return text;
    ...
    Stats stats() const {
        ...
        for (const auto &i : result) {
            switch (i.operation) {
                case DiffOp_Equal:  ret.equal += i.text.size(); break;
                case DiffOp_Insert: ret.inserted += i.text.size(); break;
                case DiffOp_Delete: ret.deleted += i.text.size(); break;
            }
        }
        return ret;
    }
