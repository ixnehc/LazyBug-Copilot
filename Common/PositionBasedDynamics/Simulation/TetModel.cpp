...
void TetModel::attachVisMesh(const ParticleData &pd)
{
	...
	const unsigned int nFaces = m_surfaceMesh.numFaces();
 	...
 	const i_math::vector3df *normals = m_surfaceMesh.getVertexNormals().data();
	...
	#pragma omp parallel default(shared)
	{
		...
		{
			const i_math::vector3df &p = m_visVertices.getPosition(i);
			...
			i_math::vector3df curBary[nNearstT];
			i_math::vector3df curInter[nNearstT];
			for (unsigned int j = 0; j < nFaces; j++)
			{
				...
				const i_math::vector3df & a = pd.getPosition0(indexA);
				const i_math::vector3df & b = pd.getPosition0(indexB);
				const i_math::vector3df & c = pd.getPosition0(indexC);

				i_math::vector3df inter, bary;
			...
			for (int k = 0; k < nNearstT; k++)
			{
				...
				const i_math::vector3df& p = m_visVertices.getPosition(i);
				const i_math::vector3df n1 = -normals[faces[3 * curT[k] + 0]];
				const i_math::vector3df n2 = -normals[faces[3 * curT[k] + 1]];
				const i_math::vector3df n3 = -normals[faces[3 * curT[k] + 2]];
				const i_math::vector3df& p1 = pd.getPosition0(faces[3 * curT[k] + 0] + m_indexOffset);
				const i_math::vector3df& p2 = pd.getPosition0(faces[3 * curT[k] + 1] + m_indexOffset);
				const i_math::vector3df& p3 = pd.getPosition0(faces[3 * curT[k] + 2] + m_indexOffset);
				const i_math::vector3df U = p.cross(n1);
				const i_math::vector3df V = p.cross(n2);
				const i_math::vector3df W = p.cross(n3);
				const i_math::vector3df UU = n1.cross(p1);
				const i_math::vector3df VV = n2.cross(p2);
				const i_math::vector3df WW = n3.cross(p3);
				const i_math::vector3df UV = (n2.cross(p1)) + (n1.cross(p2));
				const i_math::vector3df UW = (n3.cross(p1)) + (n1.cross(p3));
				const i_math::vector3df VW = (n3.cross(p2)) + (n2.cross(p3));
				// F(u,v) = F + Fu*u + Fv*v + Fuu*u*u + Fuv*u*v + Fvv*v*v == 0!
				const i_math::vector3df F = W + WW;
				const i_math::vector3df Fu = U + UW - W - WW * 2.0;
				const i_math::vector3df Fv = V + VW - W - WW * 2.0;
				const i_math::vector3df Fuu = UU - UW + WW;
				const i_math::vector3df Fuv = UV - UW - VW + WW * 2.0;
				const i_math::vector3df Fvv = VV - VW + WW;
				Real u = curBary[k][0];
				Real v = curBary[k][0];
				...
				i_math::vector3df curInter = p1*u + p2*v + p3*w;
				Real dist = (p - curInter).norm();

				i_math::vector3df n = n1*u + n2*v + n3*w;
				Real err = dist;
				if ((p - curInter).dot(n) < 0.0)
					dist *= -1.0;
				i_math::vector3df interP = curInter + n*dist;
				err += (interP - p).norm();
				...
				current_k = k;
				current_dist = dist;
				current_bary = i_math::vector3df(u, v, w);
...
void TetModel::solveQuadraticForZero(const i_math::vector3df& F, const i_math::vector3df& Fu, const i_math::vector3df& Fv, const i_math::vector3df& Fuu,
	const i_math::vector3df&Fuv, const i_math::vector3df& Fvv, Real& u, Real& v)
{
	...
	for (k = 0; k < 50; k++)
	{
		...
		// => dF^T*dF*dx = dF^T*(-F)
		// solve for dx
		const i_math::vector3df f = -(F + Fu*u + Fv*v + Fuu*u*u + Fuv*u*v + Fvv*v*v);
		if ((fabsf(f[0]) < eps) && (fabsf(f[1]) < eps) && (fabsf(f[2]) < eps))
			break;
		i_math::vector3df dF[2];
...
void TetModel::updateVisMesh(const ParticleData &pd)
{
	...
	#pragma omp parallel default(shared)
	{
		...
		for (int i = 0; i < (int) m_attachments.size(); i++)
		{
			...
			const i_math::vector3df &a = pd.getPosition(indexA);
			const i_math::vector3df &b = pd.getPosition(indexB);
			const i_math::vector3df &c = pd.getPosition(indexC);
			i_math::vector3df p2 = bary[0] * a + bary[1] * b + bary[2] * c;
			i_math::vector3df n = bary[0] * normals[faces[3 * triindex]] + bary[1] * normals[faces[3 * triindex + 1]] + bary[2] * normals[faces[3 * triindex + 2]];
			n.normalize();

			i_math::vector3df &p = m_visVertices.getPosition(pindex);
 ...
 bool TetModel::pointInTriangle(const i_math::vector3df& p0, const i_math::vector3df& p1, const i_math::vector3df& p2, const i_math::vector3df& p,
 	i_math::vector3df& inter, i_math::vector3df &bary)
 {
 	// see Bridson: Robust treatment of collisions contact and friction for cloth animation
 	const i_math::vector3df x43 = p - p2;
 	const i_math::vector3df x13 = p0 - p2;
 	const i_math::vector3df x23 = p1 - p2;
