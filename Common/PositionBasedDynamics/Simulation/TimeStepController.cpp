...
// 			TimeIntegration::semiImplicitEulerRotation(h, od.getMass(i), od.getInvMass(i) * Matrix3r::Identity() ,od.getQuaternion(i), od.getVelocity(i), i_math::vector3df(0,0,0));
// 		}
	}
...
//		#pragma omp for schedule(static) 
		for (int i = 0; i < (int) pd.size(); i++)
		{
			...
			if (i==1)
			{
				float len=pd.getVelocity(i).getLength();
				if (len>10.0f)
				{
					int v=0;
					v++;
...
// 	SimulationModel::RigidBodyVector &rb = model.getRigidBodies();
	...
	SimulationModel::SimpleContactConstraintVector &simpleContacts = model.getSimpleConstactConstraintVector();
...
//				#pragma omp for schedule(static) 
				for (int i = 0; i < groupSize; i++)
				{
					...
					if (constraints[constraintIndex]->isEnabled())
					{
		...
		for (unsigned int i = 0; i < simpleContacts.size(); i++)
		{
			if (simpleContacts[i]->isEnabled())
				simpleContacts[i]->solvePositionConstraint(model, m_iterations);
		}
