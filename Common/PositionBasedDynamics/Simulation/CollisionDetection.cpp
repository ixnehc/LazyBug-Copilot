...
void CollisionDetection::addParticleSolidContact(const unsigned int particleIndex, const unsigned int solidIndex, 
	const unsigned int tetIndex, const i_math::vector3df &bary, const i_math::vector3df &cp1, const i_math::vector3df &cp2, 
	const i_math::vector3df &normal, const Real dist, const Real restitutionCoeff, const Real frictionCoeff)
{
	if (m_solidContactCB)
...
void CollisionDetection::updateAABB(const i_math::vector3df &p, AABB &aabb)
{
	if (aabb.m_p[0][0] > p[0])
		aabb.m_p[0][0] = p[0];
	if (aabb.m_p[0][1] > p[1])
