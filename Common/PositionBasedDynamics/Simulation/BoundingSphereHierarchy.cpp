...
i_math::vector3df const& PointCloudBSH::entity_position(unsigned int i) const
{
    return m_vertices[i];
}
...
{
	auto vertices_subset = std::vector<i_math::vector3df>(n);
...
void PointCloudBSH::compute_hull_approx(unsigned int b, unsigned int n, BoundingSphere& hull) const
{
	...
	hull.x() = x;
	hull.r() = sqrtf(radius2);
}

void PointCloudBSH::init(const i_math::vector3df *vertices, const unsigned int numVertices)
{
...
i_math::vector3df const& TetMeshBSH::entity_position(unsigned int i) const
{
	return m_com[i];
}
...
void TetMeshBSH::compute_hull_approx(unsigned int b, unsigned int n, BoundingSphere& hull) const
{
	...
	hull.x() = x;
	hull.r() = sqrtf(radius2) + m_tolerance;
}

void TetMeshBSH::init(const i_math::vector3df *vertices, const unsigned int numVertices, const unsigned int *indices, const unsigned int numTets, const Real tolerance)
{
