...
namespace PBD
{
	...
	public:
		...
		void traverse_depth_first(TraversalPredicate pred, TraversalCallback cb,
			TraversalPriorityLess const& pless = NULL) const;
		void traverse_breadth_first(TraversalPredicate const& pred, TraversalCallback const& cb, unsigned int start_node = 0, TraversalPriorityLess const& pless = NULL, TraversalQueue& pending = TraversalQueue()) const;
	...
	protected:
		...
		void traverse_breadth_first(TraversalQueue& pending,
			TraversalPredicate const& pred, TraversalCallback const& cb, TraversalPriorityLess const& pless = NULL) const;

		unsigned int add_node(unsigned int b, unsigned int n);

		virtual i_math::vector3df const& entity_position(unsigned int i) const = 0;
