	...
	)
{
	...
	const i_math::vector3df &com2 = rb2->getPosition();
	...
	const i_math::vector3df &v1 = rb2->getTransformationV1();
	const i_math::vector3df &v2 = rb2->getTransformationV2();
	...
	std::function<bool(unsigned int, unsigned int)> predicate = [&](unsigned int node_index, unsigned int depth)
	{
		...
		const i_math::vector3df &sphere_x = bs.x();
		const i_math::vector3df sphere_x_w = rb1->getRotation() * sphere_x + rb1->getPosition();
		...
		if (dist < bs.r())
		{
			...
			const i_math::vector3df x = R * (sphere_x_w - com2) + v1;
	...
	std::function<void(unsigned int, unsigned int)> cb = [&](unsigned int node_index, unsigned int depth)
	{
		...
		for (auto i = node.begin; i < node.begin + node.n; ++i)
		{
			...
			const i_math::vector3df &x_w = vd.getPosition(index);
			const i_math::vector3df x = R * (x_w - com2) + v1;
			i_math::vector3df cp, n;
			Real dist;
			if (co2->collisionTest(x, m_tolerance, cp, n, dist))
			{
				const i_math::vector3df cp_w = R.transpose() * cp + v2;
				const i_math::vector3df n_w = R.transpose() * n;
	...
	)
{
	...
	{
		const BoundingSphere &bs = bvh.hull(node_index);
		const i_math::vector3df &sphere_x_w = bs.x();
	...
	std::function<void(unsigned int, unsigned int)> cb = [&](unsigned int node_index, unsigned int depth)
	{
		...
		for (auto i = node.begin; i < node.begin + node.n; ++i)
		{
			...
			const i_math::vector3df &x_w = pd.getPosition(index);
			const i_math::vector3df x = R * (x_w - com2) + v1;
			i_math::vector3df cp, n;
			Real dist;
			if (co2->collisionTest(x, m_tolerance, cp, n, dist))
	...
	std::function<void(unsigned int, unsigned int)> cb = [&](unsigned int node_index1, unsigned int node_index2)
	{
		...
		for (auto i = node1.begin; i < node1.begin + node1.n; ++i)
		{
			for (auto j = node2.begin; j < node2.begin + node2.n; ++j)
			{
				...
				const i_math::vector3df &x0 = pd.getPosition(indices[4 * tetIndex] + offset2);
				const i_math::vector3df &x1 = pd.getPosition(indices[4 * tetIndex + 1] + offset2);
				const i_math::vector3df &x2 = pd.getPosition(indices[4 * tetIndex + 2] + offset2);
				const i_math::vector3df &x3 = pd.getPosition(indices[4 * tetIndex + 3] + offset2);
				...
				A.col(1) = x2 - x0;
				A.col(2) = x3 - x0;
				i_math::vector3df bary = A.inverse() * (x_w - x0);
 					...
 					(bary[0] + bary[1] + bary[2] <= 1.0))
				{
					// use barycentric coordinates to determine position of the point in the reference space of the tet
 					const i_math::vector3df &X0 = pd.getPosition0(indices[4 * tetIndex] + offset2);
					const i_math::vector3df &X1 = pd.getPosition0(indices[4 * tetIndex + 1] + offset2);
					const i_math::vector3df &X2 = pd.getPosition0(indices[4 * tetIndex + 2] + offset2);
					const i_math::vector3df &X3 = pd.getPosition0(indices[4 * tetIndex + 3] + offset2);
					...
					const i_math::vector3df X = X0 + A0 * bary;

					i_math::vector3df cp_l, n_l;
					Real dist;

					// apply inverse initial transform to transform the point in the space of the 
					// signed distance field
					const i_math::vector3df X_l = (tm2->getInitialR().transpose() * (X - tm2->getInitialX()));
					...
					if (co2->collisionTest(X_l, 0.0, cp_l, n_l, dist))
					{
						...
						i_math::vector3df cp_bary;

						// transform closest point on surface back to the reference space of the tet model
						const i_math::vector3df cp0 = (tm2->getInitialR() * cp_l + tm2->getInitialX());
						...
						if (findRefTetAt(pd, tm2, co2, cp0, cp_tetIndex, cp_bary))
						{
							...
							i_math::vector3df cp_w;
							if (cp_tetIndex != tetIndex)
							{
								const i_math::vector3df &x0 = pd.getPosition(indices[4 * cp_tetIndex] + offset2);
								const i_math::vector3df &x1 = pd.getPosition(indices[4 * cp_tetIndex + 1] + offset2);
								const i_math::vector3df &x2 = pd.getPosition(indices[4 * cp_tetIndex + 2] + offset2);
								const i_math::vector3df &x3 = pd.getPosition(indices[4 * cp_tetIndex + 3] + offset2);
								A.col(0) = x1 - x0;
								A.col(1) = x2 - x0;
								A.col(2) = x3 - x0;
...
void DistanceFieldCollisionDetection::addCollisionBox(const unsigned int bodyIndex, const unsigned int bodyType, const i_math::vector3df *vertices, const unsigned int numVertices, const i_math::vector3df &box, const bool testMesh, const bool invertSDF)
{
...
void DistanceFieldCollisionDetection::addCollisionSphere(const unsigned int bodyIndex, const unsigned int bodyType, const i_math::vector3df *vertices, const unsigned int numVertices, const Real radius, const bool testMesh, const bool invertSDF)
{
...
void DistanceFieldCollisionDetection::addCollisionTorus(const unsigned int bodyIndex, const unsigned int bodyType, const i_math::vector3df *vertices, const unsigned int numVertices, const i_math::vector2df &radii, const bool testMesh, const bool invertSDF)
{
...
void DistanceFieldCollisionDetection::addCollisionCylinder(const unsigned int bodyIndex, const unsigned int bodyType, const i_math::vector3df *vertices, const unsigned int numVertices, const i_math::vector2df &dim, const bool testMesh, const bool invertSDF)
{
...
void DistanceFieldCollisionDetection::addCollisionHollowSphere(const unsigned int bodyIndex, const unsigned int bodyType, const i_math::vector3df *vertices, const unsigned int numVertices, const Real radius, const Real thickness, const bool testMesh, const bool invertSDF)
{
...
void DistanceFieldCollisionDetection::addCollisionHollowBox(const unsigned int bodyIndex, const unsigned int bodyType, const i_math::vector3df *vertices, const unsigned int numVertices, const i_math::vector3df &box, const Real thickness, const bool testMesh, const bool invertSDF)
{
...
void DistanceFieldCollisionDetection::addCollisionObjectWithoutGeometry(const unsigned int bodyIndex, const unsigned int bodyType, const i_math::vector3df *vertices, const unsigned int numVertices, const bool testMesh)
{
...
double DistanceFieldCollisionDetection::DistanceFieldCollisionBox::distance(const Eigen::Vector3d &x, const Real tolerance)
{
	...
	const Eigen::Vector3d d(fabsf(x_d.x()) - box_d.x(), fabsf(x_d.y()) - box_d.y(), fabsf(x_d.z()) - box_d.z());
...
bool DistanceFieldCollisionDetection::DistanceFieldCollisionSphere::collisionTest(const i_math::vector3df &x, const Real tolerance, i_math::vector3df &cp, i_math::vector3df &n, Real &dist, const Real maxDist)
{
	const i_math::vector3df d = x;
...
double DistanceFieldCollisionDetection::DistanceFieldCollisionTorus::distance(const Eigen::Vector3d &x, const Real tolerance)
{
	...
	const Eigen::Vector2d q(i_math::vector2df(x.x(), x.z()).norm() - radii_d.x(), x.y());
...
{
	const double l = sqrtf(x.x()*x.x() + x.z()*x.z());
	const Eigen::Vector2d d = Eigen::Vector2d(fabsf(l), fabsf(x.y())) - m_dim.template cast<double>();
...
double DistanceFieldCollisionDetection::DistanceFieldCollisionHollowSphere::distance(const Eigen::Vector3d &x, const Real tolerance)
{
	const Eigen::Vector3d d = x.template cast<double>();
	const double dl = d.norm();
	return m_invertSDF*(fabsf(dl - static_cast<double>(m_radius)) - static_cast<double>(m_thickness)) - static_cast<double>(tolerance);
}

bool DistanceFieldCollisionDetection::DistanceFieldCollisionHollowSphere::collisionTest(const i_math::vector3df &x, const Real tolerance, i_math::vector3df &cp, i_math::vector3df &n, Real &dist, const Real maxDist)
{
	const i_math::vector3df d = x;
	const Real dl = d.norm();
	dist = m_invertSDF*(fabsf(dl - m_radius) - m_thickness) - tolerance;
	if (dist < maxDist)
	{
		if (dl < 1.e-6)
			n.setZero();
...
double DistanceFieldCollisionDetection::DistanceFieldCollisionHollowBox::distance(const Eigen::Vector3d &x, const Real tolerance)
{
	...
	return m_invertSDF * (fabsf(std::min(d.maxCoeff(), 0.0) + max_d.norm()) - m_thickness) - static_cast<double>(tolerance);
}

void DistanceFieldCollisionDetection::DistanceFieldCollisionObject::approximateNormal(const Eigen::Vector3d &x, const Real tolerance, i_math::vector3df &n)
{
...
bool DistanceFieldCollisionDetection::DistanceFieldCollisionObject::collisionTest(const i_math::vector3df &x, const Real tolerance, i_math::vector3df &cp, i_math::vector3df &n, Real &dist, const Real maxDist)
{
...
void DistanceFieldCollisionDetection::DistanceFieldCollisionObject::initTetBVH(const i_math::vector3df *vertices, const unsigned int numVertices, const unsigned int *indices, const unsigned int numTets, const Real tolerance)
{
...
bool DistanceFieldCollisionDetection::findRefTetAt(const ParticleData &pd, TetModel *tm, const DistanceFieldCollisionDetection::DistanceFieldCollisionObject *co, const i_math::vector3df &X,
	unsigned int &tetIndex, i_math::vector3df &barycentricCoordinates)
{
 	...
 	const unsigned int offset = tm->getIndexOffset();
	std::vector<i_math::vector3df> bary;
 	...
 	std::function<void(unsigned int, unsigned int)> cb = [&](unsigned int node_index, unsigned int depth)
 	{
 		...
 		for (auto i = node.begin; i < node.begin + node.n; ++i)
 		{
			const unsigned int tetIndex = bvh0.entity(i);
 			...
 			const i_math::vector3df &X0 = pd.getPosition0(indices[4 * tetIndex] + offset);
 			const i_math::vector3df &X1 = pd.getPosition0(indices[4 * tetIndex + 1] + offset);
 			const i_math::vector3df &X2 = pd.getPosition0(indices[4 * tetIndex + 2] + offset);
 			const i_math::vector3df &X3 = pd.getPosition0(indices[4 * tetIndex + 3] + offset);
