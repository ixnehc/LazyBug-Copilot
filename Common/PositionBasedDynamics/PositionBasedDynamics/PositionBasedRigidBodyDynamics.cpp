...
void PositionBasedRigidBodyDynamics::computeMatrixK(
	const i_math::vector3df &connector,
	const Real invMass,
	const i_math::vector3df &x,
	const Matrix3r &inertiaInverseW,
	Matrix3r &K)
{
	if (invMass != 0.0)
	{
		const i_math::vector3df v = connector - x;
		const Real a = v[0];
		const Real b = v[1];
		const Real c = v[2];
...
// ----------------------------------------------------------------------------------------------
void PositionBasedRigidBodyDynamics::computeMatrixK(
	const i_math::vector3df &connector0,
	const i_math::vector3df &connector1,
	const Real invMass,
	const i_math::vector3df &x,
	const Matrix3r &inertiaInverseW,
	Matrix3r &K)
{
	if (invMass != 0.0)
	{
		const i_math::vector3df v0 = connector0 - x;
		const Real a = v0[0];
		const Real b = v0[1];
		const Real c = v0[2];

		const i_math::vector3df v1 = connector1 - x;
		const Real d = v1[0];
		const Real e = v1[1];
		const Real f = v1[2];
...
// ----------------------------------------------------------------------------------------------
void PositionBasedRigidBodyDynamics::computeMatrixG(const i_math::quatf &q, Eigen::Matrix<Real, 4, 3, Eigen::DontAlign> &G)
{
...
// ----------------------------------------------------------------------------------------------
void PositionBasedRigidBodyDynamics::computeMatrixQ(const i_math::quatf &q, Eigen::Matrix<Real, 4, 4, Eigen::DontAlign> &Q)
{
	Q(0, 0) = q.w();
	Q(0, 1) = -q.x();
	Q(0, 2) = -q.y();
...
// ----------------------------------------------------------------------------------------------
void PositionBasedRigidBodyDynamics::computeMatrixQHat(const i_math::quatf &q, Eigen::Matrix<Real, 4, 4, Eigen::DontAlign> &Q)
{
	Q(0, 0) = q.w();
	Q(0, 1) = -q.x();
	Q(0, 2) = -q.y();
...
bool PositionBasedRigidBodyDynamics::init_BallJoint(
	const i_math::vector3df &x0, 						
	const i_math::quatf &q0,					
	const i_math::vector3df &x1, 						
	const i_math::quatf &q1,					
	const i_math::vector3df &ballJointPosition,		
...
bool PositionBasedRigidBodyDynamics::solve_BallJoint(
	...
	i_math::vector3df &corr_x0, i_math::quatf &corr_q0,
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	// jointInfo contains
	// 0:	connector in body 0 (local)
	// 1:	connector in body 1 (local)
	...
	const i_math::vector3df &connector0 = ballJointInfo.col(2);
	const i_math::vector3df &connector1 = ballJointInfo.col(3);
	...
	const i_math::vector3df pt = (K1 + K2).llt().solve(connector1 - connector0);

	if (invMass0 != 0.0)
	{
		const i_math::vector3df r0 = connector0 - x0;
		corr_x0 = invMass0*pt;

		const i_math::vector3df ot = (inertiaInverseW0 * (r0.cross(pt)));
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
		corr_q0.coeffs() = 0.5 *(otQ*q0).coeffs();
	}

	if (invMass1 != 0.0)
	{
		const i_math::vector3df r1 = connector1 - x1;
		corr_x1 = -invMass1*pt;

		const i_math::vector3df ot = (inertiaInverseW1 * (r1.cross(-pt)));
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
...
bool PositionBasedRigidBodyDynamics::init_DistanceJoint(
	...
	const i_math::vector3df &x1,
	const i_math::quatf &q1,
	const i_math::vector3df &pos0,
	const i_math::vector3df &pos1,
	...
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	...
	const i_math::vector3df &c0 = jointInfo.col(2);
	const i_math::vector3df &c1 = jointInfo.col(3);
	...
	Real Kinv = 0.0;
	if (fabsf(K) > static_cast<Real>(1e-6))
	...
	lambda += delta_lambda;
	const i_math::vector3df pt = dir * delta_lambda;


	if (invMass0 != 0.0)
	{
		const i_math::vector3df r0 = c0 - x0;
		corr_x0 = invMass0 * pt;
	...
	if (invMass1 != 0.0)
	{
		const i_math::vector3df r1 = c1 - x1;
		corr_x1 = -invMass1 * pt;
...
bool PositionBasedRigidBodyDynamics::init_BallOnLineJoint(
	...
	const i_math::vector3df &direction,
	...
	)
{
	...
	i_math::vector3df v(1.0, 0.0, 0.0);
	// check if vectors are parallel
	if (fabsf(v.dot(jointInfo.col(7))) > 0.99)
		v = i_math::vector3df(0.0, 1.0, 0.0);
	...
	)
{
	...
	const i_math::vector3df dir = jointInfo.col(7);
	const i_math::vector3df p = jointInfo.col(5);
	const i_math::vector3df s = jointInfo.col(6);
	...
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	...
	const i_math::vector3df &connector0 = jointInfo.col(5);
	const i_math::vector3df &connector1 = jointInfo.col(6);
	...
	const i_math::vector2df pt2D = K.llt().solve(P * (connector1 - connector0));
	const i_math::vector3df pt = PT * pt2D;
	...
	)
{
	...
	i_math::quatf qR0(R0);
	const i_math::quatf q00 = (q0.conjugate() * qR0).conjugate();
	const i_math::quatf q10 = (q1.conjugate() * qR0).conjugate();
	...
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	...
	const i_math::vector3df &c0 = jointInfo.block<3, 1>(0, 4);
	const i_math::vector3df &c1 = jointInfo.block<3, 1>(0, 5);
	const Eigen::Matrix<Real, 2, 4> &Pr = jointInfo.block<4, 2>(0, 0).transpose();

	Eigen::Matrix<Real, 5, 1> C;
	C.block<3, 1>(0, 0) = c0 - c1;

	const i_math::quatf tmp = (q0.conjugate() * q1);
	...
	const i_math::vector3df pt = lambda.block<3, 1>(0, 0);
	const i_math::vector3df amt = t.transpose() * lambda.block<2, 1>(3, 0);

	if (invMass0 != 0.0)
	{
		corr_x0 = invMass0*pt;
		const i_math::vector3df ot = (inertiaInverseW0 * (r0.cross(pt) + amt));
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
		corr_q0.coeffs() = 0.5 *(otQ*q0).coeffs();
	}
	...
	{
		corr_x1 = -invMass1*pt;
		const i_math::vector3df ot = (inertiaInverseW1 * (r1.cross(-pt) - amt));
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
		corr_q1.coeffs() = 0.5 *(otQ*q1).coeffs();
	}
...
bool PositionBasedRigidBodyDynamics::init_UniversalJoint(
	...
	const i_math::quatf &q1,
	const i_math::vector3df &position,
	const i_math::vector3df &jointAxis0,
	const i_math::vector3df &jointAxis1,
	...
	)
{
	...
	// determine constraint coordinate system
	i_math::vector3df constraintAxis = jointAxis0.cross(jointAxis1);
	...
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	...
	const i_math::vector3df &c0 = jointInfo.col(4);
	const i_math::vector3df &c1 = jointInfo.col(5);
	const i_math::vector3df &axis0 = jointInfo.col(6);
	const i_math::vector3df &axis1 = jointInfo.col(7);
	const i_math::vector3df u = axis0.cross(axis1);
	const i_math::vector3df r0 = c0 - x0;
	const i_math::vector3df r1 = c1 - x1;
	Matrix3r r0_star, r1_star;
	MathFunctions::crossProductMatrix(r0, r0_star);
	...
	if (invMass1 != 0.0)
	{
		...
		K.block<3, 3>(0, 0) += K11;
		const i_math::vector3df K_03 = -r1_star * inertiaInverseW1 * u;
	...
	{
		corr_x0 = invMass0*pt;
		const i_math::vector3df ot = (inertiaInverseW0 * (r0.cross(pt) + u*lambda(3, 0)));
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
		corr_q0.coeffs() = 0.5 *(otQ*q0).coeffs();
	}
	...
	{
		corr_x1 = -invMass1*pt;
		const i_math::vector3df ot = (inertiaInverseW1 * (r1.cross(-pt) - u*lambda(3, 0)));
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
		corr_q1.coeffs() = 0.5 *(otQ*q1).coeffs();
	}
	...
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	...
	const i_math::vector2df &d = jointInfo.block<2, 1>(0, 2);
	...
	const i_math::vector3df pt = P.transpose() * lambda.block<2, 1>(0, 0);
	const i_math::vector3df amt = t.transpose() * lambda.block<3, 1>(2, 0);

	if (invMass0 != 0.0)
	{
		corr_x0 = invMass0 * pt;
		const i_math::vector3df ot = inertiaInverseW0 * amt;
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
		corr_q0.coeffs() = 0.5 *(otQ*q0).coeffs();
	}
	...
	{
		corr_x1 = -invMass1 * pt;
		const i_math::vector3df ot = -inertiaInverseW1 * amt;
		const i_math::quatf otQ(0.0, ot[0], ot[1], ot[2]);
		corr_q1.coeffs() = 0.5 *(otQ*q1).coeffs();
	}
	...
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	...
	const i_math::vector3df &axis = jointInfo.block<3, 1>(0, 1);

	const Eigen::Matrix<Real, 3, 4> &Pr = jointInfo.block<4, 3>(0, 3).transpose();

	const i_math::vector3df &d = jointInfo.block<3, 1>(0, 2);
	...
	const i_math::vector3df pt = lambda.block<3, 1>(0, 0);
	const i_math::vector3df amt = t.transpose() *  lambda.block<3, 1>(3, 0);
...
bool PositionBasedRigidBodyDynamics::velocitySolve_TargetVelocityMotorSliderJoint(
	const Real invMass0,
	const i_math::vector3df &x0,
	const i_math::vector3df &v0,
	const Matrix3r &inertiaInverseW0,
	const i_math::quatf &q0,
	const i_math::vector3df &omega0,
	const Real invMass1,
	const i_math::vector3df &x1,
	const i_math::vector3df &v1,
	const Matrix3r &inertiaInverseW1,
	const i_math::quatf &q1,
	const i_math::vector3df &omega1,
	const Real targetVelocity,
	const Eigen::Matrix<Real, 4, 6, Eigen::DontAlign> &jointInfo,
	i_math::vector3df &corr_v0, i_math::vector3df &corr_omega0,
	i_math::vector3df &corr_v1, i_math::vector3df &corr_omega1)
{
	// jointInfo contains
	// 0:	coordinate system in body 0, where the x-axis is the slider axis (local)
	// 1:	coordinate system in body 0, where the x-axis is the slider axis (global)
	...
	const i_math::vector3df &axis0 = R0.col(0);

	// evaluate constraint function
	i_math::vector3df deltaOmega = omega0 - omega1;
	...
	const i_math::vector3df p = lambda.block<3, 1>(0, 0);
	const i_math::vector3df angMomentum = lambda.block<3, 1>(3, 0);
	...
	i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
{
	...
	// 6:	connector in body 1 (global)
	// 7:	hinge axis in body 0 (local) used for rendering 
	const i_math::vector3df &c0 = jointInfo.block<3, 1>(0, 5);
	const i_math::vector3df &c1 = jointInfo.block<3, 1>(0, 6);
	...
	const Vector4r qVec(tmp.w(), tmp.x(), tmp.y(), tmp.z());
	C.block<3, 1>(3, 0) = Pr * qVec;
	C(3, 0) -= sinf(static_cast<Real>(0.5)*targetAngle);
	...
	i_math::vector3df &corr_v1, i_math::vector3df &corr_omega1)
{
	...
	const i_math::vector3df &axis0 = jointInfo.block<3, 1>(0, 7);
	...
	i_math::vector3df angMomentum = lambda.block<3, 1>(3, 0);
		...
		i_math::vector3df &corr_x1, i_math::quatf &corr_q1)
	{
		...
		const i_math::vector3df pt = R0 * delta_lambda.block<3, 1>(0, 0);
		const i_math::vector3df amt = t.transpose() *  delta_lambda.block<3, 1>(3, 0);
...
bool PositionBasedRigidBodyDynamics::solve_RigidBodyParticleBallJoint(
	...
	i_math::vector3df &corr_x0, i_math::quatf &corr_q0,
	i_math::vector3df &corr_x1)
{
	// jointInfo contains
	// 0:	connector in rigid body (local)
	// 1:	connector in rigid body (global)

	const i_math::vector3df &connector0 = jointInfo.col(1);
	...
	const i_math::vector3df pt = (K1 + K2).llt().solve(x1 - connector0);
...
bool PositionBasedRigidBodyDynamics::init_RigidBodyContactConstraint(
	...
	const i_math::vector3df &x0,						// center of mass of body 0
	const i_math::vector3df &v0,						// velocity of body 0
	const Matrix3r &inertiaInverseW0,		// inverse inertia tensor (world space) of body 0
	const i_math::quatf &q0,					// rotation of body 0	
	const i_math::vector3df &omega0,					// angular velocity of body 0
	const Real invMass1,							// inverse mass is zero if body is static
	const i_math::vector3df &x1,						// center of mass of body 1
	const i_math::vector3df &v1,						// velocity of body 1
	const Matrix3r &inertiaInverseW1,		// inverse inertia tensor (world space) of body 1
	const i_math::quatf &q1,					// rotation of body 1
	const i_math::vector3df &omega1,					// angular velocity of body 1
	const i_math::vector3df &cp0,						// contact point of body 0
	const i_math::vector3df &cp1,						// contact point of body 1
	...
	Eigen::Matrix<Real, 3, 5, Eigen::DontAlign> &constraintInfo)
{
	...
	const i_math::vector3df r0 = cp0 - x0;
	const i_math::vector3df r1 = cp1 - x1;

	const i_math::vector3df u0 = v0 + omega0.cross(r0);
	const i_math::vector3df u1 = v1 + omega1.cross(r1);
	const i_math::vector3df u_rel = u0 - u1;
	...
	i_math::vector3df &corr_v1, i_math::vector3df &corr_omega1)
{
	...
	const i_math::vector3df &connector0 = constraintInfo.col(0);
	const i_math::vector3df &connector1 = constraintInfo.col(1);
	const i_math::vector3df &normal = constraintInfo.col(2);
	const i_math::vector3df &tangent = constraintInfo.col(3);
	...
	i_math::vector3df p(correctionMagnitude * normal);
	...
	Eigen::Matrix<Real, 3, 5, Eigen::DontAlign> &constraintInfo)
{
	...
	const i_math::vector3df u1 = v1 + omega1.cross(r1);
	const i_math::vector3df u_rel = v0 - u1;
	const Real u_rel_n = normal.dot(u_rel);
