	...
	*/
	...
	public:
		...
		static bool solve_GenericConstraint(
			...
			const i_math::vector3df x[numberOfParticles],						// positions of particles
			void *userData,

			void(*constraintFct)(
				const unsigned int numParticles,
				const Real invMass[],							// inverse mass is zero if particle is static
				const i_math::vector3df x[],						// positions of particles
				void *userData,
			...
			i_math::vector3df corr_x[numberOfParticles]);		
		...
		*/
		...
		static bool solve_GenericConstraint(
			...
			const i_math::vector3df x[numberOfRigidBodies],						// positions of bodies
			const Matrix3r inertiaInverseW[numberOfRigidBodies],		// inverse inertia tensor (world space) of bodies
			const i_math::quatf q[numberOfRigidBodies],					// rotation of bodies
			void *userData,
			...
			i_math::vector3df corr_x[numberOfRigidBodies],
			i_math::quatf corr_q[numberOfRigidBodies]);
		...
		*/
		...
		static bool solve_GenericConstraint(
			...
			const i_math::vector3df x[numberOfRigidBodies],						// positions of bodies
			const Matrix3r inertiaInverseW[numberOfRigidBodies],		// inverse inertia tensor (world space) of bodies
			const i_math::quatf q[numberOfRigidBodies],
			void *userData,
		...
		*/
		 ...
		 * a 3D representation. */
		static void computeMatrixG(const i_math::quatf &q, Eigen::Matrix<Real, 4, 3> &G);
		...
		i_math::vector3df corr_x[numberOfParticles])
	{
		...
		i_math::vector3df xTemp[numberOfParticles];
		...
		i_math::quatf corr_q[numberOfRigidBodies])
	{
		...
		for (unsigned int i = 0u; i < numberOfRigidBodies; i++)
		{
			if (invMass[i] != 0.0)
			{
				...
				const i_math::vector3df ot = (inertiaInverseW[i] * pt.block<3, 1>(3, 0));
	...
	void PositionBasedGenericConstraints::computeMatrixG(const i_math::quatf &q, Eigen::Matrix<Real, 4, 3> &G)
	{
	...
	void PositionBasedGenericConstraints::approximateGradient(
		...
		Eigen::Matrix<Real, dim, 6> &jacobian)
	{
		i_math::vector3df xTemp[numberOfRigidBodies];
		i_math::quatf qTemp[numberOfRigidBodies];
