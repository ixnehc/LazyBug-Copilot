...
bool PositionBasedCosseratRods::solve_StretchShearConstraint(
	...
	const i_math::quatf& q0, Real invMassq0,
	const i_math::vector3df& stretchingAndShearingKs,
	const Real restLength,
	i_math::vector3df& corr0, i_math::vector3df& corr1, i_math::quatf& corrq0)
{
	i_math::vector3df d3;	//third director d3 = q0 * e_3 * q0_conjugate
	...
	i_math::vector3df gamma = (p1 - p0) / restLength - d3;
	...
	i_math::quatf q_e_3_bar(q0.z(), -q0.y(), q0.x(), -q0.w());	//compute q*e_3.conjugate (cheaper than quaternion product)
	corrq0 = i_math::quatf(0.0, gamma.x(), gamma.y(), gamma.z()) * q_e_3_bar;
...
bool PositionBasedCosseratRods::solve_BendTwistConstraint(
	const i_math::quatf& q0, Real invMassq0,
	const i_math::quatf& q1, Real invMassq1,
	const i_math::vector3df& bendingAndTwistingKs,
	const i_math::quatf& restDarbouxVector,
	i_math::quatf& corrq0, i_math::quatf& corrq1)
{
	i_math::quatf omega = q0.conjugate() * q1;   //darboux vector

	i_math::quatf omega_plus;
...
{
	const i_math::vector3df pm = 0.5 * (p0 + p1);
	const i_math::vector3df p0p2 = p0 - p2;
	const i_math::vector3df p2p1 = p2 - p1;
	const i_math::vector3df p1p0 = p1 - p0;
	const i_math::vector3df p2pm = p2 - pm;
	...
	i_math::vector3df& corr0, i_math::vector3df&  corr1, i_math::vector3df&  corr2)
{
	...
	i_math::vector3df pm = 0.5 * (p0 + p1);
	i_math::vector3df p2pm = p2 - pm;
...
bool PositionBasedElasticRods::solve_DarbouxVectorConstraint(
	...
	const i_math::vector3df& p4, Real invMass4,
	const i_math::vector3df& bendingAndTwistingKs,
	const Real midEdgeLength,
	const i_math::vector3df& restDarbouxVector,
	i_math::vector3df& corr0, i_math::vector3df&  corr1, i_math::vector3df&  corr2, i_math::vector3df&  corr3, i_math::vector3df& corr4)
{
	//  Single rod element:
	//      3   4		//ghost points
	//		|	|
	...
	i_math::vector3df darboux_vector;
	Matrix3r d0, d1;
	...
	const i_math::vector3df constraint_value(bendingAndTwistingKs[0] * (darboux_vector[0] - restDarbouxVector[0]),
...
// ----------------------------------------------------------------------------------------------
bool PositionBasedElasticRods::computeDarbouxVector(const Matrix3r& dA, const Matrix3r& dB, const Real mid_edge_length, i_math::vector3df& darboux_vector)
{
...
bool PositionBasedElasticRods::computeMaterialFrameDerivative(
	const i_math::vector3df& p0, const i_math::vector3df& p1, const i_math::vector3df& p2, const Matrix3r& d,
	...
	Matrix3r& d3p0, Matrix3r& d3p1, Matrix3r& d3p2)
{
	...
	// d3pi
	//////////////////////////////////////////////////////////////////////////
	const i_math::vector3df p01 = p1 - p0;
	...
	// d2pi
	//////////////////////////////////////////////////////////////////////////
	const i_math::vector3df p02 = p2 - p0;
	const i_math::vector3df p01_cross_p02 = p01.cross(p02);
...
bool PositionBasedElasticRods::computeDarbouxGradient(
	const i_math::vector3df& darboux_vector, const Real length,
	const Matrix3r& da, const Matrix3r& db,
	const Matrix3r dajpi[3][3], const Matrix3r dbjpi[3][3],
	//const i_math::vector3df& bendAndTwistKs,
	...
	)
{
	...
	for (int c = 0; c < 3; ++c) 
	{
		...
		// pa
		{
			i_math::vector3df term1(0,0,0);
			i_math::vector3df term2(0,0,0);
			i_math::vector3df tmp(0,0,0);
...
bool PBD::DirectPositionBasedSolverForStiffRods::computeDarbouxVector(const i_math::quatf & q0, const i_math::quatf & q1, const Real averageSegmentLength, i_math::vector3df & darbouxVector)
{
	darbouxVector = 2. / averageSegmentLength * (q0.conjugate() * q1).vec();
	return true;
}

bool PBD::DirectPositionBasedSolverForStiffRods::computeBendingAndTorsionJacobians(const i_math::quatf & q0, const i_math::quatf & q1, const Real averageSegmentLength, Eigen::Matrix<Real, 3, 4> & jOmega0, Eigen::Matrix<Real, 3, 4> & jOmega1)
{
	jOmega0 <<
		-q1.w(), -q1.z(), q1.y(), q1.x(),
		q1.z(), -q1.w(), -q1.x(), q1.y(),
...
bool PBD::DirectPositionBasedSolverForStiffRods::computeMatrixG(const i_math::quatf & q, Eigen::Matrix<Real, 4, 3> & G)
{
...
void PBD::DirectPositionBasedSolverForStiffRods::computeMatrixK(const i_math::vector3df &connector, const Real invMass, const i_math::vector3df &x, const Matrix3r &inertiaInverseW, Matrix3r &K)
{
	if (invMass != 0.0)
	{
...
void PBD::DirectPositionBasedSolverForStiffRods::getMassMatrix(RodSegment *segment, Matrix6r &M)
{
	...
	const i_math::vector3df &inertiaLocal = segment->InertiaTensor();
...
Real PBD::DirectPositionBasedSolverForStiffRods::factor(const int intervalIndex, const std::vector<RodConstraint*> &rodConstraints, std::vector<RodSegment*> & rodSegments, const Interval* &intervals, std::list <Node*> * forward, std::list <Node*> * backward, std::vector<Vector6r> & RHS, std::vector<Vector6r> & lambdaSums, std::vector<std::vector<Matrix3r>> & bendingAndTorsionJacobians)
{
	...
	for (size_t currentConstraintIndex = 0; currentConstraintIndex < rodConstraints.size(); ++currentConstraintIndex)
	{
		...
		const i_math::quatf &q0 = segment0->Rotation();
		const i_math::quatf &q1 = segment1->Rotation();
		...
		const i_math::vector3df &connector0 = constraintInfo.col(2);
		const i_math::vector3df &connector1 = constraintInfo.col(3);
		i_math::vector3df stretchViolation = connector0 - connector1;

		// compute Darboux vector (Equation (7))
		i_math::vector3df omega;
		...
		i_math::vector3df bendingAndTorsionViolation = omega - currentConstraint->getRestDarbouxVector();
		...
		rhs.block<3, 1>(0, 0) = -stretchViolation
			- i_math::vector3df(currentConstraint->getStretchCompliance().array() *
			lambdaSum.block<3, 1>(0, 0).array());

		rhs.block<3, 1>(3, 0) = -bendingAndTorsionViolation
			- i_math::vector3df(currentConstraint->getBendingAndTorsionCompliance().array() *
	...
	for (nodeIter = forward[intervalIndex].begin(); nodeIter != forward[intervalIndex].end(); nodeIter++)
	{
		...
		if (node->isconstraint)
		{
			...
			//insert compliance
			node->D.setZero();
			const i_math::vector3df &stretchCompliance(currentConstraint->getStretchCompliance());

			node->D(0, 0) -= stretchCompliance[0];
			node->D(1, 1) -= stretchCompliance[1];
			node->D(2, 2) -= stretchCompliance[2];

			const i_math::vector3df &bendingAndTorsionCompliance(currentConstraint->getBendingAndTorsionCompliance());
		...
		if (node->parent != NULL)
		{
			if (node->isconstraint)
			{
				...
				const i_math::vector3df r = constraintInfo.col(2 + segmentIndex) - segment->Position();
				Matrix3r r_cross;
...
bool PBD::DirectPositionBasedSolverForStiffRods::solve(int intervalIndex, std::list <Node*> * forward, std::list <Node*> * backward, std::vector<Vector6r> & RHS, std::vector<Vector6r> & lambdaSums, std::vector<i_math::vector3df> & corr_x, std::vector<i_math::quatf> & corr_q)
{
	std::list<Node*>::iterator nodeIter;
	for (nodeIter = forward[intervalIndex].begin(); nodeIter != forward[intervalIndex].end(); nodeIter++)
	{
	...
	for (nodeIter = forward[intervalIndex].begin(); nodeIter != forward[intervalIndex].end(); nodeIter++)
	{
		...
		if (!node->isconstraint)
		{
			...
			i_math::vector3df deltaXSoln = i_math::vector3df(-soln[0], -soln[1], -soln[2]);
			corr_x[node->index] = deltaXSoln;

			Eigen::Matrix<Real, 4, 3> G;
			computeMatrixG(segment->Rotation(), G);
			i_math::quatf deltaQSoln;
			deltaQSoln.coeffs() = G * i_math::vector3df(-soln[3], -soln[4], -soln[5]);
...
bool PBD::DirectPositionBasedSolverForStiffRods::init_DirectPositionBasedSolverForStiffRodsConstraint(
	...
	std::vector<i_math::vector3df> & corr_x, 
	std::vector<i_math::quatf> & corr_q
	)
{
	// init constraints
	...
	std::fill(corr_x.begin(), corr_x.end(), i_math::vector3df::Zero());

	corr_q.resize(rodSegments.size());
	std::fill(corr_q.begin(), corr_q.end(), i_math::quatf::Identity());
...
bool PBD::DirectPositionBasedSolverForStiffRods::init_StretchBendingTwistingConstraint(
	const i_math::vector3df &x0, const i_math::quatf &q0, 
	const i_math::vector3df &x1, const i_math::quatf &q1, 
	const i_math::vector3df &constraintPosition, 
	...
	i_math::vector3df &stiffnessCoefficientK, 
	i_math::vector3df &restDarbouxVector
	)
{
	...
	stiffnessCoefficientK = i_math::vector3df(bendingStiffness, torsionStiffness, bendingStiffness);
...
bool PBD::DirectPositionBasedSolverForStiffRods::initBeforeProjection_StretchBendingTwistingConstraint(
	const i_math::vector3df &stiffnessCoefficientK,
	const Real inverseTimeStepSize,
	const Real averageSegmentLength,
	i_math::vector3df &stretchCompliance,
	i_math::vector3df &bendingAndTorsionCompliance,
	Vector6r &lambdaSum
	)
{
...
bool PBD::DirectPositionBasedSolverForStiffRods::solve_StretchBendingTwistingConstraint(
	...
	const i_math::vector3df &restDarbouxVector, 
	const Real averageSegmentLength, 
	const i_math::vector3df &stretchCompliance, 
	const i_math::vector3df &bendingAndTorsionCompliance, 
	...
	)
{
	...
	i_math::vector3df bendingAndTorsionViolation = omega - restDarbouxVector;

	// fill right hand side of the linear equation system (Equation (19))
	Vector6r rhs;
	rhs.block<3, 1>(0, 0) = -stretchViolation
		- i_math::vector3df(stretchCompliance.array() * lambdaSum.block<3, 1>(0, 0).array());

	rhs.block<3, 1>(3, 0) = -bendingAndTorsionViolation
		- i_math::vector3df(bendingAndTorsionCompliance.array() * lambdaSum.block<3, 1>(3, 0).array());
	...
	const i_math::vector3df ra = connector0 - x0;
	const i_math::vector3df rb = connector1 - x1;
	...
	i_math::vector3df deltaLambdaStretch(deltaLambda.block<3, 1>(0, 0)),
