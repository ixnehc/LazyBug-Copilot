...
bool PositionBasedFluids::computePBFDensity(
	...
	const Real mass[],
	const i_math::vector3df boundaryX[],
	...
	Real &lambda)
{
	...
	if (C != 0.0)
	{
		...
		i_math::vector3df gradC_i(0.0, 0.0, 0.0);
		...
		for (unsigned int j = 0; j < numNeighbors; j++)
		{
			...
			if (neighborIndex < numberOfParticles)		// Test if fluid particle
			{
				const i_math::vector3df gradC_j = -mass[neighborIndex] / density0 * CubicKernel::gradW(x[particleIndex] - x[neighborIndex]);
			...
			{
				// Boundary: Akinci2012
				const i_math::vector3df gradC_j = -boundaryPsi[neighborIndex - numberOfParticles] / density0 * CubicKernel::gradW(x[particleIndex] - boundaryX[neighborIndex - numberOfParticles]);
				sum_grad_C2 += gradC_j.squaredNorm();
				gradC_i -= gradC_j;
			}
		}
