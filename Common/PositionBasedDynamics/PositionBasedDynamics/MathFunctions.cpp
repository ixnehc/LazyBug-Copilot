...
// 	Real t = static_cast<Real>(1.0) / (fabsf(d) + sqrtf(d*d + static_cast<Real>(1.0)));
// 	if (d < 0.0) t = -t;
// 	Real c = static_cast<Real>(1.0) / sqrtf(t*t + 1);
// 	Real s = t*c;
// 	A(p, p) += t*A(p, q);
// 	A(q, q) -= t*A(p, q);
// 	A(p, q) = A(q, p) = 0.0;
...
// ----------------------------------------------------------------------------------------------
// void MathFunctions::eigenDecomposition(const Matrix3r &A, Matrix3r &eigenVecs, i_math::vector3df &eigenVals)
// {
...
// 
// 	Real d0 = sqrtf(eigenVals[0]);
// 	Real d1 = sqrtf(eigenVals[1]);
// 	Real d2 = sqrtf(eigenVals[2]);
// 	D.setZero();
// 	D(0, 0) = d0;
// 	D(1, 1) = d1;
// 	D(2, 2) = d2;
...
// {
// 	const Real sum1 = fabsf(A(0,0)) + fabsf(A(1,0)) + fabsf(A(2,0));
// 	const Real sum2 = fabsf(A(0,1)) + fabsf(A(1,1)) + fabsf(A(2,1));
// 	const Real sum3 = fabsf(A(0,2)) + fabsf(A(1,2)) + fabsf(A(2,2));
// 	Real maxSum = sum1;
// 	if (sum2 > maxSum)
// 		maxSum = sum2;
// 	if (sum3 > maxSum)
...
// {
// 	const Real sum1 = fabsf(A(0, 0)) + fabsf(A(0, 1)) + fabsf(A(0, 2));
// 	const Real sum2 = fabsf(A(1, 0)) + fabsf(A(1, 1)) + fabsf(A(1, 2));
// 	const Real sum3 = fabsf(A(2, 0)) + fabsf(A(2, 1)) + fabsf(A(2, 2));
// 	Real maxSum = sum1;
// 	if (sum2 > maxSum)
// 		maxSum = sum2;
// 	if (sum3 > maxSum)
...
// 
// 		if (fabsf(det) < 1.0e-12)
// 		{
// 			i_math::vector3df len;
...
// 
// 		const Real gamma = sqrtf(sqrtf((MadjTone*MadjTinf) / (Mone*Minf)) / fabsf(det));
// 
...
*/
// void MathFunctions::svdWithInversionHandling(const Matrix3r &A, i_math::vector3df &sigma, Matrix3r &U, Matrix3r &VT)
// {
// 
// 	Matrix3r AT_A, V;
// 	AT_A = A.transpose() * A;
...
// 	}
// 
// 	if (S[0] < 0.0) S[0] = 0.0;		// safety for sqrtf
// 	if (S[1] < 0.0) S[1] = 0.0;
// 	if (S[2] < 0.0) S[2] = 0.0;
// 
// 	sigma[0] = sqrtf(S[0]);
...
// 			}
// 
// 			i_math::vector3df v[2];
// 			unsigned char index = 0;
// 			for (unsigned char l = 0; l < 3; l++)
// 			{
// 				if (l != pos)
// 				{
// 					v[index++] = i_math::vector3df(U(0, l), U(1, l), U(2, l));
// 				}
// 			}
// 			i_math::vector3df vec = v[0].cross(v[1]);
// 			vec.normalize();
// 			U(0, pos) = vec[0];
// 			U(1, pos) = vec[1];
// 			U(2, pos) = vec[2];
...
// 	else
// 	{
// 		i_math::vector3df sigmaInv(static_cast<Real>(1.0) / sigma[0], static_cast<Real>(1.0) / sigma[1], static_cast<Real>(1.0) / sigma[2]);
// 		U = A * V;
// 		for (unsigned char l = 0; l < 3; l++)
// 		{
// 			for (unsigned char m = 0; m < 3; m++)
...
// ----------------------------------------------------------------------------------------------
Real MathFunctions::cotTheta(const i_math::vector3df &v, const i_math::vector3df &w)
{
	const Real cosTheta = v.dotProduct(w);
	const Real sinTheta = (v.crossProduct(w)).getLength();
	return (cosTheta / sinTheta);
}

// ----------------------------------------------------------------------------------------------
// void MathFunctions::crossProductMatrix(const i_math::vector3df &v, Matrix3r &v_hat)
// {
// 	v_hat << 0, -v(2), v(1),
// 		v(2), 0, -v(0),
// 		-v(1), v(0), 0;
...
// 
// ----------------------------------------------------------------------------------------------
// void MathFunctions::extractRotation(const Matrix3r &A, i_math::quatf &q,	const unsigned int maxIter)
// {
// 	for (unsigned int iter = 0; iter < maxIter; iter++)
// 	{
// 		Matrix3r R = q.matrix();
// 		i_math::vector3df omega = (R.col(0).cross(A.col(0)) + R.col(1).cross(A.col(1)) + R.col(2).cross(A.col(2))) * 
// 			(1.0 / fabsf(R.col(0).dot(A.col(0)) + R.col(1).dot(A.col(1)) + R.col(2).dot(A.col(2)) + 1.0e-9));
// 		Real w = omega.norm();
// 		if (w < 1.0e-9)
// 			break;
// 		q = i_math::quatf(AngleAxisr(w, (1.0 / w) * omega)) *	q;
// 		q.normalize();
// 	}
// }