...
float Triangulater::Area(const std::vector< i_math::vector2df> &contour)
{
  ...
  return A*0.5f;
   ...
   /*
     ...
     defined by A, B, C.
   */
bool Triangulater::InsideTriangle(float Ax, float Ay,
                      float Bx, float By,
                      float Cx, float Cy,
                      float Px, float Py)
...
bool Triangulater::Snip(const std::vector< i_math::vector2df> &contour,int u,int v,int w,int n,int *V)
{
  int p;
...
bool Triangulater::Process(const std::vector< i_math::vector2df> &contour,std::vector< i_math::vector2df> &result)
{
  /* allocate and initialize list of Vertices in polygon */
	...
	for(int m=0, v=nv-1; nv>2; )
	{
		...
		if (0 >= (count--))
		{
			//** Triangulater: ERROR - probable bad polygon!
			return false;
		}
		...
		if ( Snip(contour,u,v,w,nv,V.data()) )
		{
			int a,b,c,s,t;
...
// 
//   std::vector< i_math::vector2df> result;
// 
//   //  Invoke the triangulator to triangulate this polygon.
//   Triangulater::Process(a,result);
// 
