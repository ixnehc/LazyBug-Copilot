...
extern char g_strbuffer[4096];

void PrintCell(YExcel::BasicExcelCell* cell)
{
	switch (cell->Type())
	{
	...
	case YExcel::BasicExcelCell::INT:
		sprintf(g_strbuffer,"%10d", cell->GetInteger());
		break;

	case YExcel::BasicExcelCell::DOUBLE:
		sprintf(g_strbuffer,"%10.6lf", cell->GetDouble());
		break;

	case YExcel::BasicExcelCell::STRING:
		sprintf(g_strbuffer,"%10s", cell->GetString());
		break;
	case YExcel::BasicExcelCell::WSTRING:
		{
			const wchar_t *wc=cell->GetWString();
			int sz=wcslen(wc);
			sz=WideCharToMultiByte(0,0,cell->GetWString(),sz,g_strbuffer,sizeof(g_strbuffer),"",NULL);
			g_strbuffer[sz]=0;
			break;
		}
	}
...
BOOL DumpDBTable3(const char *pathDB,const char *table,std::string &result)
{
	YExcel::BasicExcel e;

	e.Load(pathDB);
	result="";
	YExcel::BasicExcelWorksheet* sheet1=NULL;
	if (table[0])
	{
		sheet1= e.GetWorksheet(table);
		if (!sheet1)
		{
			//把table转成unicode试试
			wchar_t buf[256];
			int c=MultiByteToWideChar(0,0,table,strlen(table),buf,ARRAY_SIZE(buf));
			buf[c]=0;
			sheet1=e.GetWorksheet(buf);
		}
	}
	else
	{
		if (e.GetTotalWorkSheets()>0)
			sheet1=e.GetWorksheet((size_t)0);
	}
	...
	if (sheet1)
	{
		...
		if (TRUE)
		{
			...
			for (size_t c=0; c<maxCols; ++c)
			{
				YExcel::BasicExcelCell* cell = sheet1->Cell(0,c);
				PrintCell(cell);
				if (g_strbuffer[0])//有内容
					n=c+1;
			}
			maxCols=n;
		}
		...
		{
			if (TRUE)//看这一行是不是被注释掉了
			{
				YExcel::BasicExcelCell* cell = sheet1->Cell(r,0);
				PrintCell(cell);
				if ((g_strbuffer[0]=='/')&&(g_strbuffer[1]=='/'))
					continue;
			}
			for (size_t c=0; c<maxCols; ++c)
			{
