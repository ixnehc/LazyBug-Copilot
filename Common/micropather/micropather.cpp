...
{
  public:
	OpenQueue( MPGraph* _graph )
	{ 
		graph = _graph; 
		sentinel = (MPPathNode*) sentinelMem;
	...
	void Push( MPPathNode* pNode );
	MPPathNode* Pop();
	void Update( MPPathNode* pNode );
  ...
  private:
	...
	MPPathNode* sentinel;
	int sentinelMem[ ( sizeof( MPPathNode ) + sizeof( int ) ) / sizeof( int ) ];
	MPGraph* graph;	// for debugging
};


void OpenQueue::Push( MPPathNode* pNode )
{
...
MPPathNode* OpenQueue::Pop()
{
	MPASSERT( sentinel->next != sentinel );
	MPPathNode* pNode = sentinel->next;
	pNode->Unlink();
#ifdef DEBUG
	sentinel->CheckList();
#endif
...
void OpenQueue::Update( MPPathNode* pNode )
{
#ifdef DEBUG_PATH_DEEP
	printf( "Open Update: " );		
	graph->PrintStateInfo( pNode->state );
...
{
  public:
	ClosedSet( MPGraph* _graph )		{ this->graph = _graph; }
	~ClosedSet()	{}

	void Add( MPPathNode* pNode )
	{
		#ifdef DEBUG_PATH_DEEP
			printf( "Closed add: " );		
			graph->PrintStateInfo( pNode->state );
...
MPPathNodePool::MPPathNodePool( unsigned _allocate, unsigned _typicalAdjacent ) 
	: firstBlock( 0 ),
	  blocks( 0 ),
	  ...
	  nAvailable( 0 )
{
	...
	cacheSize = 0;
	cache = (MPNodeCost*)malloc(cacheCap * sizeof(MPNodeCost));
	...
	while( HashSize() < allocate )
		++hashShift;
#endif
	hashTable = (MPPathNode**)calloc( HashSize(), sizeof(MPPathNode*) );
...
bool MPPathNodePool::PushCache( const MPNodeCost* nodes, int nNodes, int* start ) {
	*start = -1;
...
void MPPathNodePool::Clear()
{
	...
	if ( nAllocated > 0 ) {
		...
		memset( hashTable, 0, sizeof(MPPathNode*)*HashSize() );
...
MPPathNodePool::Block* MPPathNodePool::NewBlock()
{
	Block* block = (Block*) calloc( 1, sizeof(Block) + sizeof(MPPathNode)*(allocate-1) );
	block->nextBlock = 0;
...
unsigned MPPathNodePool::Hash( void* voidval ) 
{
	/*
...
MPPathNode* MPPathNodePool::Alloc()
{
	...
	MPPathNode* pathNode = freeMemSentinel.next;
	pathNode->Unlink();
...
void MPPathNodePool::AddPathNode( unsigned key, MPPathNode* root )
{
	if ( hashTable[key] ) {
		MPPathNode* p = hashTable[key];
		while( true ) {
...
MPPathNode* MPPathNodePool::GetPathNode( unsigned frame, void* _state, float _costFromStart, float _estToGoal, MPPathNode* _parent )
{
	unsigned key = Hash( _state );

	MPPathNode* root = hashTable[key];
	while( root ) {
...
void MPPathNode::Init(	unsigned _frame,
						void* _state,
...
MicroPather::MicroPather( MPGraph* _graph, unsigned allocate, unsigned typicalAdjacent )
...
void MicroPather::GoalReached( MPPathNode* node, void* start, void* end, std::vector< void* > *_path )
{
...
void MicroPather::GetNodeNeighbors( MPPathNode* node, std::vector< MPNodeCost >* pNodeCost )
{
	...
	else if ( node->cacheIndex < 0 )
	{
		...
		if ( node->numAdjacent > 0 ) {
			...
			const MPStateCost* stateCostVecPtr = &stateCostVec.at(0);
			MPNodeCost* pNodeCostPtr = &pNodeCost->at(0);
		...
		for( int i=0; i<node->numAdjacent; ++i ) {
			MPPathNode* pNode = pNodeCostPtr[i].node;
...
void MPPathNodePool::AllStates( unsigned frame, std::vector< void* >* stateVec )
{	
...
int MicroPather::Solve( void* startNode, void* endNode, std::vector< void* >* path, float* cost )
{
	...
	*cost = 0.0f;
	...
	MPPathNode* newPathNode = pathNodePool.GetPathNode(	frame, 
														startNode, 
														0, 
														graph->LeastCostEstimate( startNode, endNode ), 
														0 );
	...
	while ( !open.Empty() )
	{
		...
		else
		{
			...
			for( int i=0; i<node->numAdjacent; ++i )
			{
				...
				MPPathNode* child = nodeCostVec[i].node;
				float newCost = node->costFromStart + nodeCostVec[i].cost;

				MPPathNode* inOpen   = child->inOpen ? child : 0;
				MPPathNode* inClosed = child->inClosed ? child : 0;
				MPPathNode* inEither = (MPPathNode*)( ((MP_UPTR)inOpen) | ((MP_UPTR)inClosed) );
...
int MicroPather::SolveForNearStates( void* startState, std::vector< MPStateCost >* nr, float maxCost )
{
	/*	 http://en.wikipedia.org/wiki/Dijkstra%27s_algorithm

		 1  function Dijkstra(MPGraph, source):
		 2      for each vertex v in MPGraph:           // Initializations
		 3          dist[v] := infinity               // Unknown distance function from source to v
		 4          previous[v] := undefined          // Previous node in optimal path from source
		 5      dist[source] := 0                     // Distance from source to source
		 6      Q := the set of all nodes in MPGraph
	...
	*/
	...
	MPPathNode closedSentinel;
	closedSentinel.Clear();
	closedSentinel.Init( frame, 0, FLT_MAX, FLT_MAX, 0 );
	closedSentinel.next = closedSentinel.prev = &closedSentinel;

	MPPathNode* newPathNode = pathNodePool.GetPathNode( frame, startState, 0, 0, 0 );
	open.Push( newPathNode );
	
	while ( !open.Empty() )
	{
		MPPathNode* node = open.Pop();	// smallest dist
		...
		for( int i=0; i<node->numAdjacent; ++i )
		{
			...
			MPPathNode* inOpen   = nodeCostVec[i].node->inOpen ? nodeCostVec[i].node : 0;
			MPPathNode* inClosed = nodeCostVec[i].node->inClosed ? nodeCostVec[i].node : 0;
			MPASSERT( !( inOpen && inClosed ) );
			MPPathNode* inEither = inOpen ? inOpen : inClosed;
			MPASSERT( inEither != node );
	...
	for( MPPathNode* pNode=closedSentinel.next; pNode != &closedSentinel; pNode=pNode->next ) {
...
#ifdef DEBUG
	for( unsigned i=0; i<nr->size(); ++i ) {
		for( unsigned k=i+1; k<nr->size(); ++k ) {
			MPASSERT( nr->at(i).state != nr->at(k).state );
		}
	}
#endif
