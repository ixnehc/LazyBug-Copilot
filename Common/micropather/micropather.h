...
class MPGraph
{
  public:
	...
	*/	
	virtual void AdjacentCost( void* state, std::vector< MPStateCost > *adjacent ) = 0;
...
/*
	Every state (void*) is represented by a MPPathNode in MicroPather. There
	can only be one MPPathNode for a given state.
*/
class MPPathNode
{
  public:
	...
	void Clear() {
		memset( this, 0, sizeof( MPPathNode ) );
		numAdjacent = -1;
		cacheIndex  = -1;
	}
	void InitSentinel() {
	...
	MPPathNode* parent;		// the parent is used to reconstruct the path
	...
	MPPathNode *child[2];		// Binary search in the hash table. [left, right]
	MPPathNode *next, *prev;	// used by open queue
	...
	void AddBefore( MPPathNode* addThis ) {
		addThis->next = this;
		addThis->prev = prev;
		prev->next = addThis;
		prev = addThis;
	...
	void CheckList()
	{
		...
		for( MPPathNode* it = next; it != this; it=it->next ) {
  ...
  private:
	...
	void operator=( const MPPathNode& );
};


/* Memory manager for the PathNodes. */
class MPPathNodePool
{
public:
	MPPathNodePool( unsigned allocate, unsigned typicalAdjacent );
	~MPPathNodePool();
	...
	//		pNode = New();
	//
	// Get the MPPathNode associated with this state. If the MPPathNode already
	// exists (allocated and is on the current frame), it will be returned. 
	// Else a new MPPathNode is allocated and returned. The returned object
	// is always fully initialized.
	//
	// NOTE: if the pathNode exists (and is current) all the initialization
	//       parameters are ignored.
	MPPathNode* GetPathNode(		unsigned frame,
								void* _state,
								float _costFromStart, 
								float _estToGoal, 
								MPPathNode* _parent );
	...
	bool PushCache( const MPNodeCost* nodes, int nNodes, int* start );

	// Get neighbors from the cache
	// Note - always access this with an offset. Can get re-allocated.
	void GetCache( int start, int nNodes, MPNodeCost* nodes ) {
		MPASSERT( start >= 0 && start < cacheCap );
		MPASSERT( nNodes > 0 );
		MPASSERT( start + nNodes <= cacheCap );
		memcpy( nodes, &cache[start], sizeof(MPNodeCost)*nNodes );
	}
...
class MicroPather
{
  ...
  public:
	...
	/**
		...
		the MPGraph callbacks.

		@param graph		The "map" that implements the MPGraph callbacks.
		...
		@param typicalAdjacent	Used to determine cache size. The typical number of adjacent states
								...
								more memory.
	*/
	MicroPather( MPGraph* graph, unsigned allocate = 250, unsigned typicalAdjacent=6 );
	~MicroPather();
	...
	*/
	int SolveForNearStates( void* startState, std::vector< MPStateCost >* near, float maxCost );
  ...
  private:
	...
	void GoalReached( MPPathNode* node, void* start, void* end, std::vector< void* > *path );

	void GetNodeNeighbors(	MPPathNode* node, std::vector< MPNodeCost >* neighborNode );

	#ifdef DEBUG
	//void DumpStats();
	#endif

	MPPathNodePool				pathNodePool;
	std::vector< MPStateCost >	stateCostVec;	// local to Solve, but put here to reduce memory allocation
	std::vector< MPNodeCost >		nodeCostVec;	// local to Solve, but put here to reduce memory allocation
