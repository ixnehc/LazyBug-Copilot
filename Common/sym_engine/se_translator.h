...
public:
	...
	public:
		uInt						code() const			{ return m_pexp->ExceptionRecord->ExceptionCode; }
		void *						address() const			{ return m_pexp->ExceptionRecord->ExceptionAddress; }
		const EXCEPTION_POINTERS *	info() const			{ return m_pexp; }
		cChar *						name() const			{ return se_translator::name(code()); }
		cChar *						description() const		{ return se_translator::description(code()); }
		virtual cChar *				caption() const			{ return name();}
	...
	class access_violation : public exception
	{
	 ...
	 public:
		 ...
		 cChar *	caption() const{sprintf(m_buf,"%s(%s:0x%08x)",name(),(is_read_op()?"read":"write"),inaccessible_address());return m_buf;}
	private:
		mutable char m_buf[60];
	};
	...
	class no_memory : public exception
	{
	 ...
	 public:
		 ...
		 cChar *	caption() const{sprintf(m_buf,"%s(size:0x%08x)",name(),mem_size());return m_buf;}
	private:
		mutable char m_buf[60];
	};
...
//	DECLARE_EXCEPTION_CLASS(access_violation)
 ...
 public:
	...
	static void translator(uInt, EXCEPTION_POINTERS * pexp);
	static cChar *	name(uInt code);
	static cChar *	description(uInt code);
