...
/*
 * Modified by Paul Chitescu <paulc-devel@null.ro>
 */
#include "stdh.h"
...
void TiXmlElement::SetAttribute( const std::string& name, int value )
{	
	char buf[64];
	sprintf( buf, "%d", value );
...
void TiXmlElement::SetAttribute( const std::string& name, unsigned int value )
{	
	char buf[64];
	sprintf( buf, "%d", value );
...
void TiXmlElement::SetAttribute( const std::string& name, float value )
{	
	char buf[64];
	sprintf( buf, "%f", value );
...
void TiXmlElement::PrintToString( std::string &s, int depth )
{
	int i;
	for ( i=0; i<depth; i++ )
		s+="    " ;
...
bool TiXmlDocument::LoadRawData(unsigned char *pData)
{
	Clear();
	Parse((const char *) pData );
...
void TiXmlDocument::PrintToString(std::string &s,int depth)
{
	TiXmlNode* node;
	for ( node=FirstChild(); node; node=node->NextSibling() )
	{
		node->PrintToString(s, 0 );
		s+=0x0d;
		s+=0x0a;
	}
}
...
void TiXmlAttribute::PrintToString(std::string &s, int )
{
...
void TiXmlComment::PrintToString(std::string &s, int depth )
{
	for ( int i=0; i<depth; i++ )
		s+="    ";
	s+="<!--";
...
void TiXmlText::PrintToString( std::string &s, int )
{
	s+=value;
}


void TiXmlCData::Print( FILE* fp, int depth)
{
	for ( int i=0; i<depth; i++ )
		fprintf( fp, "    " );
	fprintf( fp, "<![CDATA[%s]]>", value.c_str() );
}

void TiXmlCData::PrintToString(std::string &s, int depth)
{
	for ( int i=0; i<depth; i++ )
		s+="    ";
	s+="<![CDATA[";
...
void TiXmlDeclaration::PrintToString( std::string &s, int )
{
	std::string out = "<?xml ";
...
void TiXmlUnknown::PrintToString( std::string &s, int depth )
{
	for ( int i=0; i<depth; i++ )
		s+="    ";
	s+=value;
