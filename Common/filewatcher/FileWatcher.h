...
const int FW_NOTIFYRESULT_LENGTH	= 81920;
...
public:
	BOOL Initialize()
	{
		_pNotifyResult = new char[FW_NOTIFYRESULT_LENGTH];
...
public:
	BOOL IsStarted();
	BOOL Start(const char* targetDirectory, DWORD dwNotifyFilter = WNF_CHANGE_LAST_WRITE| WNF_CHANGE_FILE_NAME);
	void Stop();
...
protected:
	BOOL _SetupWatcher();
	BOOL _IsFileMatchSuffix(const char* fileName) const;
...
private:
	...
	std::vector<std::string> _suffixFilters;  // 后缀过滤列表（小写）
};
#endif