...
BIND_NPCPARAM(NPC_Follower,NPCParam_Follower);

BOOL CreateNPCs(NPCParam_Follower *param,CLevelNPCs *npcs,LevelRecordNPC *rec,NPCDistribute &distribute)
{
	int idx=CSysRandom::RandRangeInt<DWORD>(0,distribute.locs.size());
	LosNPCLoc *loc=distribute.locs[idx];

	NPC_Follower *npc=Class_New2(NPC_Follower);
...
void NPC_Follower::_OnCreate()
{

}

void NPC_Follower::_OnDestroy()
{
...
void NPC_Follower::_SaveEntry(Entry *entry,LevelPlayerStates *lps)
{
...
void NPC_Follower::_LoadEntry(Entry *entry,LevelPlayerStates *lps)
{
	LPSNpcData *p=NULL;
	if (TRUE)
	{
	...
	{
		NPCParam_Follower *param=(NPCParam_Follower *)_param;
...
void NPC_Follower::_ClearEntry(Entry *entry)
{
	if (entry->bhv)
	{
		entry->bhv->Clear();
...
void NPC_Follower::_OnAddPlayer(LevelPlayerID idPlayer)
{
...
void NPC_Follower::_OnRemovePlayer(LevelPlayerID idPlayer)
{
	Entry *entry=&_entries[idPlayer];
...
void NPC_Follower::_OnUpdate(AnimTick t)
{
	for (int i=0;i<ARRAY_SIZE(_entries);i++)
	{
		Entry *entry=&_entries[i];
