...
static BashFlavor GetBashFlavor()
{
...
// 将一条 bash 命令行中出现的所有 Windows 绝对路径转换为当前 bash(WSL) 可识别的 POSIX 路径
...
static std::string ConvertBashCommandPaths(const std::string& command)
{
...
// 测试 ConvertBashCommandPaths 函数
static void TestConvertBashCommandPaths()
{
	struct TestCase {
		const char* input;
	...
	TestCase tests[] = {
		// 1. 转义空格 - Git Bash 风格裸路径
		{
			R"(grep -rn "EnumMarkers" /c/Program\ Files/Microsoft\ Visual\ Studio/2022/Community/VC/Tools/MSVC/ 2>/dev/null | head -5)",
			"Git Bash escaped spaces in bare path"
		},
		// 2. 四种格式混合
		{
			R"(grep -rn "x" "D:\LazyBug\a.h" /mnt/c/foo /c/bar /cygdrive/e/baz)",
			"Mixed path formats"
		},
		// 3. 含空格的引号路径
		{
			R"(cat "C:\Program Files\x.txt")",
			"Quoted path with spaces"
		},
		// 4. 单引号包裹的 Git Bash 风格路径
		{
			R"(ffmpeg -i '/c/My Videos/a.mp4' out.gif)",
			"Single quoted Git Bash path"
		},
		// 5. 已是 WSL 格式(应保持不变)
		{
			R"(ls /mnt/c/Users)",
		...
		// 7. 同一命令混合多种风格
		{
			R"(diff C:/a.txt /mnt/d/b.txt /c/c.txt /cygdrive/e/d.txt)",
			"Multiple path styles in one command"
		},
		// 8. 裸 Windows 反斜杠路径(无引号)
		{
			R"(find D:\src -name "*.cpp")",
			"Bare Windows backslash path"
		},
		// 9. 非路径不应误转
		{
			R"(grep -rn "x" /usr/bin /home/user /mnt/c/src)",
			"Unix paths should not be converted"
		},
		// 10. 单字母路径边界
		{
			R"(ls /c)",
			"Single drive letter path"
		},
		// 11. 复杂组合: 管道+重定向+多格式路径
		{
			R"(cat "C:\Logs\a.log" | grep error > /mnt/d/output.txt)",
			"Complex command with pipe and redirect"
		},
		// 12. 多个转义空格
		{
			R"(ls /d/My\ Documents/Project\ Files/)",
			"Multiple escaped spaces"
		},
		// 13. 混合引号和裸路径
		{
			R"(cp C:/a.txt "/mnt/d/My Docs/" /e/backup\ folder/)",
			"Mixed quoted and bare paths"
		},
	};

	OutputDebugStringA("=== ConvertBashCommandPaths Tests ===\n");
	for (size_t i = 0; i < sizeof(tests) / sizeof(tests[0]); ++i)
	{
		std::string input = tests[i].input;
		std::string output = ConvertBashCommandPaths(input);
		
		std::string msg = "Test " + std::to_string(i + 1) + ": " + tests[i].description + "\n";
		msg += "  IN:  " + input + "\n";
		msg += "  OUT: " + output + "\n\n";
...
{
	// DEBUG 模式下执行一次路径转换测试
	static bool s_testRun = true;
	if (!s_testRun)
	{
		s_testRun = true;
		TestConvertBashCommandPaths();
	}
	...
	else if (shellType == "bash.exe")
	{
		...
		// 将命令行中出现的 Windows 绝对路径(如 C:\... 或 D:/...)转换为 WSL 可识别的 /mnt/c/ 路径
		cleanCommand = ConvertBashCommandPaths(cleanCommand);
		...
		{
			cmdLine = "bash.exe -c \"" + ConvertBashCommandPaths(command) + "\"";
		}
	}
