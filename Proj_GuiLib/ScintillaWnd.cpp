...
{
	if (!CWnd::CreateEx(0, STR_SCINTILLAWND,lpszWindowName, dwStyle, rect, pParentWnd,(UINT)nID))
   {   
      LPVOID lpMsgBuf;
...
void CScintillaWnd::SetText (
                             LPCSTR szText,BOOL bSetFocus) //@parm pointer to new text
{
   LRESULT lResult = 0;
   if (szText != NULL)
...
void CScintillaWnd::GetText (
							 std::string&strText) //@parm handle to receive text
{
                                           ...
                                           LPCSTR szExtension) //@parm filename extension without dot e.g. "cpp"
{
   ...
   while (pExtension != NULL)
   {
      ...
      {
		  if (st.Next().CompareNoCase(fromMBCS(szExtension)) == 0)
            return nFormats[i];
      }
      i++;
...
int CScintillaWnd::SetMarginWidth(int width)
{
	SendMessage(SCI_SETMARGINWIDTHN,1, width);
	return TRUE;
}
...
void CScintillaWnd::EmptyUndoBuffer()
{
...
void CScintillaWnd::SetCurrentPosition(int pos)
{
	SendMessage(SCI_SETCURRENTPOS,pos);
}
...
void CScintillaWnd::SetHotSpot(int nStyle, BOOL bFlag)
{
	SendMessage(SCI_STYLESETHOTSPOT, nStyle, (long)bFlag);
}
...
void CScintillaWnd::DefineBookmark(int marker,DWORD colFg,DWORD colBg,int shape)
{
	SendMessage(SCI_MARKERDEFINE, marker, shape);
	SendMessage(SCI_MARKERSETFORE,marker,colFg);
	SendMessage(SCI_MARKERSETBACK,marker,colBg);
}
...
void CScintillaWnd::AddBookmark(
                                long lLine,int marker) //@parm line where to add bookmark - lines start at 1
{
   SendMessage(SCI_MARKERADD, lLine, marker);
}
/////////////////////////////////////
...
void CScintillaWnd::DeleteBookmark(
                                   long lLine,int marker) //@parm line where to delete bookmark - lines start at 1
{
   if (HasBookmark(lLine,marker))
   {
      SendMessage(SCI_MARKERDELETE, lLine, marker);
   }
}

void CScintillaWnd::DeleteAllBookmark(int marker)
{
	int iLine;
	while((iLine=FindNextBookmark(1<<marker,-1))>=0)
		DeleteBookmark(iLine,marker);
}
...
BOOL CScintillaWnd::HasBookmark(
                                long lLine,int marker) //@parm line where to add bookmark - lines start at 1
{
   int n = SendMessage(SCI_MARKERGET, lLine, marker);
...
// @rvalue void | not used
//
int CScintillaWnd::FindNextBookmark(DWORD markers,int iStart,BOOL bGoThere)
{
//	long lLine = SendMessage(SCI_MARKERNEXT, iStart==-1?GetCurrentLine():iStart, markers);
	long lLine = SendMessage(SCI_MARKERNEXT, iStart, markers);
	if ((lLine >= 0) && (bGoThere))
...
// @rvalue void | not used
//
int CScintillaWnd::FindPreviousBookmark(DWORD markers,int iStart,BOOL bGoThere)
{
	long lLine = SendMessage(SCI_MARKERPREVIOUS, iStart==-1?GetCurrentLine()-1:iStart, markers);
   if ((lLine >= 0)&&(bGoThere))
      SendMessage(SCI_GOTOLINE, lLine,0);
   return lLine;
}
...
//
int CScintillaWnd::SearchForward(LPSTR szText,long lPosStart,BOOL bGoThere)
{
   if (szText == NULL)
      return -1;
...
void CScintillaWnd::SetKeywords(int set,LPCSTR keywords)
{
	SendMessage(SCI_SETKEYWORDS, set,reinterpret_cast<LPARAM>(keywords));
}

void CScintillaWnd::SetStyle(int style, COLORREF fore, COLORREF back, int size, const char *face)
{
...
void CScintillaWnd::SetCaretFore(DWORD col)
{
	SendMessage(SCI_SETCARETFORE, col,0);
}

void CScintillaWnd::SetSelColor(COLORREF fore, COLORREF back)
{
	SendMessage(SCI_SETSELFORE,1,fore);
	SendMessage(SCI_SETSELBACK,1,back);
}
...
//
const char * CScintillaWnd::GetSelectedText()
{
...
void CScintillaWnd::SetWordChar(const char *chars)
{
	SendMessage(SCI_SETWORDCHARS,0, (LPARAM)chars);
}

 
const char *CScintillaWnd::GetCurWord(int pos_,const char *wordchars)
{
	int pos;
	if (pos_==-1)
	...
	if (wordchars[0])
		SendMessage(SCI_SETWORDCHARS,0, (LPARAM)wordchars);

	int start,end;
	start=SendMessage(SCI_WORDSTARTPOSITION,pos,1);
	end=SendMessage(SCI_WORDENDPOSITION,pos,1);
	...
	*p = '\0';
	SendMessage(SCI_GETTEXTRANGE, 0, (LPARAM)&range);
	static CString strReturn;
	strReturn=p;
	delete [] p;

// 	if (wordchars)
// 		SendMessage(SCI_SETWORDCHARS,0,0);

	return (LPCTSTR)strReturn;
}

// @cmember get the word at the current position
const char *CScintillaWnd::GetCurText()
{
	char *p;
	p=(char *)GetSelectedText();
	if (p[0]==0)
		return GetCurWord();
	else
		return p;
...
BOOL CScintillaWnd::SearchForwardAuto()
{
	CString s;
	s=GetCurText();
	if (s=="")
...
void CScintillaWnd::SetTipFore(DWORD col)
{
	SendMessage(SCI_CALLTIPSETFORE,col);
}

void CScintillaWnd::SetTipBack(DWORD col)
{
	SendMessage(SCI_CALLTIPSETBACK,col);
}


void CScintillaWnd::ShowTip(int pos,const char *content)
{
	if (this!=GetFocus())
		return;

	SendMessage(SCI_CALLTIPSHOW,pos, (LPARAM)content);
}

void CScintillaWnd::HideTip()
{
	SendMessage(SCI_CALLTIPCANCEL,0,0);

}

void CScintillaWnd::SetDwellTime(DWORD tick)
{
	SendMessage(SCI_SETMOUSEDWELLTIME,tick,tick);
}

int CScintillaWnd::GetCharAt(int pos)
{
	return SendMessage(SCI_GETCHARAT,pos);
}

void CScintillaWnd::OnKillFocus(CWnd*wnd )
{
...
void CScintillaWnd::EnablePopupMenu(BOOL bEnable)
{
	SendMessage(SCI_USEPOPUP,(DWORD)bEnable);
}

int CScintillaWnd::PositionFromPoint(int x,int y)
{
	return SendMessage(SCI_POSITIONFROMPOINTCLOSE,x,y);
}

int CScintillaWnd::PositionFromLine(int iLine)
{
	return SendMessage(SCI_POSITIONFROMLINE,iLine);
}

int CScintillaWnd::EndPosFromLine(int iLine)
{
	return SendMessage(SCI_GETLINEENDPOSITION,iLine);
}


int CScintillaWnd::LineFromPosition(int pos)
{
	return SendMessage(SCI_LINEFROMPOSITION,pos);
}

const char *CScintillaWnd::GetLineText(int iLine)
{
	int length=SendMessage(SCI_GETLINEENDPOSITION,iLine,iLine)-SendMessage(SCI_POSITIONFROMLINE,iLine,iLine);

	if (length<=0)
		return "";

	static std::vector<char>buf;
	buf.resize(length+1+20);//+20以防万一
	SendMessage(SCI_GETLINE,iLine,(LPARAM)buf.data());
	buf[length]=0;

	return buf.data();
}

void CScintillaWnd::AddText(const char *str)
{
	int len=strlen(str);

	SendMessage(SCI_ADDTEXT,len, (LPARAM)str);

}


void CScintillaWnd::InsertText(const char *str,int pos)
{
	SendMessage(SCI_INSERTTEXT,pos, (LPARAM)str);
}


void CScintillaWnd::AppendText(const char *str)
{
	int len=strlen(str);

	SendMessage(SCI_APPENDTEXT,len, (LPARAM)str);
}

int CScintillaWnd::GetLineCount()
{
	return SendMessage(SCI_GETLINECOUNT,0,0);
}


void CScintillaWnd::AC_Show(int lenEntered,const char *list)
{

	SendMessage(SCI_AUTOCSHOW,lenEntered, (LPARAM)list);
}

void CScintillaWnd::AC_Cancel()
{
	SendMessage(SCI_AUTOCCANCEL,0,0);
}

BOOL CScintillaWnd::AC_IsActive()
{
	return SendMessage(SCI_AUTOCACTIVE,0,0)!=0;
}

void CScintillaWnd::AC_Complete()
{
	SendMessage(SCI_AUTOCCOMPLETE,0,0);
}

void CScintillaWnd::AC_SetStopChars(const char *chars)
{
	SendMessage(SCI_AUTOCSTOPS,0, (LPARAM)chars);
}

void CScintillaWnd::AC_SetMaxHeight(int row)
{
	SendMessage(SCI_AUTOCSETMAXHEIGHT,row,0);
}

/////////////////////////////////////
// @mfunc Set the code page (encoding) for the document e.g. SC_CP_UTF8
// @rvalue void | not used
//
void CScintillaWnd::SetCodePage(int codePage)
{
	SendMessage(SCI_SETCODEPAGE, codePage, 0);
}

/////////////////////////////////////
// @mfunc Set selection range by character position
// @rvalue void | not used
//
void CScintillaWnd::SetSelection(
									long startPos, //@parm selection start position
									long endPos)   //@parm selection end position
{
	SendMessage(SCI_SETSEL, startPos, endPos);
}

/////////////////////////////////////
// @mfunc Set selection range using line and column (0-based)
// @rvalue void | not used
//
void CScintillaWnd::SetSelection(
									int startLine,     //@parm selection start line (0-based)
									int startColumn,   //@parm selection start column (0-based)
									int endLine,       //@parm selection end line (0-based)
									int endColumn)     //@parm selection end column (0-based)
{
	long startPos = SendMessage(SCI_FINDCOLUMN, startLine, startColumn);
	long endPos = SendMessage(SCI_FINDCOLUMN, endLine, endColumn);

	// Check if positions are valid before setting selection
	if (startPos != -1 && endPos != -1)
	{
		SendMessage(SCI_SETSEL, startPos, endPos);
	}
	// Optionally, handle invalid line/column numbers here, e.g., log an error or do nothing.
}


long CScintillaWnd::GetFirstVisibleLine()
{
	return SendMessage(SCI_GETFIRSTVISIBLELINE, 0, 0);
}

void CScintillaWnd::SetFirstVisibleLine(int line)
{
	SendMessage(SCI_SETFIRSTVISIBLELINE, line, 0);
}

void CScintillaWnd::SetCurrentLine(long line)
{
	// 获取目标行的位置
	long pos = SendMessage(SCI_POSITIONFROMLINE, line, 0);
	...
	if (pos >= 0)
	{
		...
		// 同时设置选择锚点到相同位置，以清除任何选择
		SendMessage(SCI_SETANCHOR, pos, 0);
		
		// 确保该行可见
		SendMessage(SCI_ENSUREVISIBLE, line, 0);
		
		// 将该行滚动到视图中央
		int linesOnScreen = SendMessage(SCI_LINESONSCREEN, 0, 0);
		int topLine = line - linesOnScreen / 2;
		if (topLine < 0)
		{
			topLine = 0;
		}
		SendMessage(SCI_SETFIRSTVISIBLELINE, topLine, 0);
	}
}
...
void CScintillaWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	nmhdr.code = SCN_CUSTOM_POST_KEYDOWN;
	...
	// 发送通知消息给父窗口
	GetParent()->SendMessage(WM_NOTIFY, nmhdr.idFrom, (LPARAM)&scn);
}
