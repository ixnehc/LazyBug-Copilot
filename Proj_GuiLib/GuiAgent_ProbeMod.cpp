...
CGuiAgent_ProbeMod::CGuiAgent_ProbeMod(Matrix_EditMode mode)
...
CGuiAgent_ProbeMod::~CGuiAgent_ProbeMod(void)
{
	SAFE_RELEASE(_mtrl);
	SAFE_RELEASE(_mesh);
	SAFE_RELEASE(_light);
...
BOOL CGuiAgent_ProbeMod::OnDraw()
{	
...
BOOL CGuiAgent_ProbeMod::_TouchRes()
{
...
BOOL CGuiAgent_ProbeMod::OnLButtonDown(int x,int y,DWORD flag)
{
...
void CGuiAgent_ProbeMod::_DrawAbbCorner(i_math::aabbox3df &abb)
{
	IRenderPort * rp = GetRP();
	assert(rp);
...
//////////////////////////////////////////////////////////////////////////
void * CGuiAgent_ProbeMod::_GetSelBuf()
{
	GuiData_El * data = (GuiData_El *)FindData("envlight");
	if(!data)
		return NULL;
...
i_math::matrix43f * CGuiAgent_ProbeMod::_GetMat(H3DNode node)
{
...
i_math::pos2di * CGuiAgent_ProbeMod:: _GetBlock(H3DNode node)
{
	IProbeCubeMapEditor * editor = _GetEditor();
	if(!editor)
		return NULL;
...
float CGuiAgent_ProbeMod::_Clamp(float v,float ov)
{
	float rv = v;
...
void CGuiAgent_ProbeMod::_Move(H3DNode &node,i_math::matrix43f &mat)
{
	IProbeCubeMapEditor * editor = _GetEditor();
	if(!editor)
		return;
	...
	hObj = editor->SetInfo(hObj,info);
	info.aabb.getCorners(_corners);

	node = H3DNode(hObj);
}
void CGuiAgent_ProbeMod::_EndMatrixEdit(i_math::matrix43f *mat)
{	
...
IProbeCubeMapEditor * CGuiAgent_ProbeMod::_GetEditor()
{	
	GuiData_El * data = (GuiData_El *)FindData("envlight");
	if(!data)
		return NULL;
...
DWORD * CGuiAgent_ProbeMod::_GetVer()
{
	GuiData_El * data = (GuiData_El *)FindData("envlight");
	if(data)
		return &(data->ver);
