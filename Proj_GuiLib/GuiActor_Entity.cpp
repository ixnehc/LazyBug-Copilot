...
void CMod_ChangeEntityMap::OnBackup()
{
	...
	SetAddOnData(buf.data(),buf.size());
}
	...
	for (int i=0;i<nEns;i++)
	{
		...
		for (int j=0;j<nSrcMat;j++)
		{
			for (int k=0;k<nAligners;k++)
			{
				...
				if (TRUE)
				{
					i_math::vector3df dirSrc(0,0,1);
					i_math::vector3df dirDest(0,0,1);
					matSrc[j].rotateVect(dirSrc,dirSrc);
					mat2.rotateVect(dirDest,dirDest);

					dirSrc.normalize();
					dirDest.normalize();

					i_math::quatf q;
					q.from2Vector(dirSrc,dirDest);

					i_math::vector3df v;
					q.toAngleAxis(radGap,v);
					radGap=fabsf(radGap);
				}

				if ((dist<2.0f)&&(radGap<30.0f*i_math::GRAD_PI2))
				{
					if (dist<distMin)
					{
						distMin=dist;
...
//			if (_bCtrlOrShift)
		...
		else
		{
			...
			else
			{
				...
				float dist=sqrtf((float)(off.x*off.x+off.y*off.y));
				_angle=-dist*0.01f;
				_rotAxis.set(0,1,0);
				if (dist>=0.01f)
				{
...
BOOL CGuiAgent_OperateEntity::OnRButtonClick(int x,int y,DWORD flag)
{
    ...
    {
        _AddMenuSep();
        _AddMenu("根据选择对象生成Proto...", ID_AGENT_MAKE_PROTO_FROM_SELECTION);
		_AddMenu("拆解选择对象", ID_AGENT_EXPLODE_PROTO_FROM_SELECTION);
		if (data->selections.size()==1)
			_AddMenu("在ProtoEditor中打开", ID_AGENT_OPENPROTO_FROM_SELECTION);
    }

	return TRUE;
}

BOOL CGuiAgent_OperateEntity::OnLButtonDblClk(int x,int y,DWORD flag)
{
	GuiData_EntityMap*data=(GuiData_EntityMap*)FindData("entitymap");
	if (!data)
		return TRUE;
	...
	{
		EntityAddress addr=data->selections[0];
		IEntityParam *ep=data->mp->ToEntityParam(addr);
		if (ep)
		{
			ProtoID id=ep->GetProtoID();

			std::string path=data->pES->GetProtoLib()->FindPath(id);
			MakeFileSuffix(path,"prt");

			CGuiPanel_Entity*owner=(CGuiPanel_Entity*)GetActor();
			if (owner)
			{
				CPrlTree &tree=owner->GetPrlTree();
				HTREEITEM hItem=tree.ItemFromPath(path);
				if (hItem!=NULL)
				{
					HTREEITEM hParentItem=tree.GetParentItem(hItem);
					if (hParentItem!=NULL)
					{
						tree.EnsureVisible(hParentItem);
						tree.SelectItem(hParentItem);
					}
				}
			}
		}
...
BOOL CGuiAgent_OperateEntity::OnCommand(DWORD idCmd)
{
	...
	if (idCmd == ID_AGENT_OPENPROTO_FROM_SELECTION)
	{
		GuiData_EntityMap*data = (GuiData_EntityMap*)FindData("entitymap");
		if (!data)
			return FALSE;
	...
	if (idCmd == ID_AGENT_EXPLODE_PROTO_FROM_SELECTION)
	{
		GuiData_EntityMap*data = (GuiData_EntityMap*)FindData("entitymap");
		if (!data)
			return FALSE;
		...
		if (data->selections.size()>0)
		{
			for (int i = 0;i < data->selections.size();i++)
			{
				...
				if (addr != EntityAddress_Null)
				{
					i_math::matrix43f *matBase=data->mp->GetEntityMat(addr);
					if (matBase)
					{
						IEntity *en = data->mp->ToEntity(addr);
						if (en)
						{
							...
							if (proto)
							{
								CNodeTree *ntreeProto = proto->GetNodeTree()->GetTree();

								if (ntreeProto)
								{
									NodeHandle hParentNode = NodeHandle_Root;

									DWORD c = ntreeProto->GetChildCount(hParentNode);
									for (int j = 0;j < c;j++)
									{
										NodeHandle hChild = ntreeProto->GetChild(hParentNode, j);

										nm = ntreeProto->GetName(hChild);

										IProtoNode *node = proto->FindNode(nm.c_str());

										if (node)
										{
											if (node->GetType() == PN_Entity)
											{
												i_math::matrix43f &matLocal = node->GetLocalMat();

												i_math::matrix43f mat = matLocal * (*matBase);

												IEntityParam *ep = data->pES->CreateEntityParam();
												ep->SetProtoID(node->GetProtoID());
												EntityAddress addrNew = data->mp->Reside(mat, ep);

												if (addrNew != EntityAddress_Null)
												{
													IEntity *enNew = data->mp->ToEntity(addr);
													if (enNew)
														RepairAssetUID(enNew, FALSE);
												}

												i_math::pos2di ptBlk;
												data->mp->ResolveBlockPos(addrNew, ptBlk);

												UNIQUE_VEC_ADD(blks, ptBlk);
											}
										}
									}
								}
					...
					if (TRUE)
					{
						...
						ep = data->mp->UnReside(mat, (EntityAddress)addr);
						SAFE_RELEASE(ep);
					}
			...
			if (blks.size()>0)
			{
				...
				{
					mod = new CMod_ChangeEntityMap(GetView());
					mod->BackupBlocks(blks.data(), blks.size());
					mod->BackupSelection();
				}
    ...
    if (idCmd == ID_AGENT_MAKE_PROTO_FROM_SELECTION)
    {
        GuiData_EntityMap*data = (GuiData_EntityMap*)FindData("entitymap");
        if (!data)
            return FALSE;
        IProtoLib *lib = data->pES->GetProtoLib();
		std::string pathRoot=lib->GetLibFolderPath();
        const char *path = FD_BrowseProto(FALSE, pathRoot.c_str());
        if (path[0])
        {
			BOOL bOk=FALSE;

            std::string pathFolder=GetFileFolderPath(std::string(path));
            CNodeTree *ntree = lib->GetNodeTree()->GetTree();
            NodeHandle hParent = ntree->Find(pathFolder.c_str());
            if (hParent != NodeHandle_Null)
            {
				std::string nameProto=GetFileName(std::string(path));
				std::string pathProto=path;
				RemoveFileSuffix(pathProto);
                NodeHandle hChild=ntree->AddChild(hParent, 2, nameProto.c_str());
                if (hChild != NodeHandle_Null)
                {
                    IProto *proto=lib->ObtainProto(pathProto.c_str());
                    if (proto)
                    {
                        CNodeTree *ntreeProto = proto->GetNodeTree()->GetTree();
                        if (ntreeProto)
                        {
                            std::string name = CProtoTree::GenNewAutoName();
                            i_math::pos2di ptBack = proto->GetNextNodePos();
                            i_math::pos2di ptStart(0, 0);
                            ...
                            if (TRUE)
                            {
                                ...
                                for (int i = 0;i < data->selections.size();i++)
                                {
                                    ...
                                    if (addr != EntityAddress_Null)
                                    {
                                        i_math::matrix43f *mat = data->mp->GetEntityMat(addr);
										if (mat)
										{
											if (c == 0)
												posCenter = mat->getTranslation();
											else
												posCenter+= mat->getTranslation();
											c++;
										}
                            ...
                            for (int i = 0;i < data->selections.size();i++)
                            {
                                ...
                                if (addr != EntityAddress_Null)
                                {
									...
									if (mat)
									{
										...
										if (en)
										{
											const char *pathSubProto=lib->FindPath(en->GetProto()->GetID());
											...
											while (!ntreeProto->CheckChildName(hParentNode, name.c_str()))
											{
												name=CProtoTree::GenUniqueAutoName();
											}

											ntreeProto->AddChild(hParentNode, PROTO_NODETYPE_PROTO, name.c_str(), (void*)pathSubProto);

											IProtoNode *node=proto->FindNode(name.c_str());
											if (node)
											{
												i_math::matrix43f matLocal=*mat;
												(*matLocal.getTranslationP()) -= posCenter;
												node->SetLocalMat(matLocal);
											}
			...
			if (!bOk)
				AfxMessageBox(_T("Some error occurs,proto not created!"));
        }
...
H3DNode CGuiAgent_OperateEntity::_Clone(H3DNode node)
{
	...
	if (data)
	{
		if (data->mp)
		{
			EntityAddress addrClone=data->mp->Clone((EntityAddress)node,i_math::vector3df(0.0f,0.0f,0.0f));
...
int CProtoThumbnailsList::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	SetIconSpacing(ProtoThumbnailWidth+2,ProtoThumbnailHeight+2);

	return 0;
}

void CProtoThumbnailsList::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	*pResult = 0;

	NodeHandle hCurSel=_GetCurSel();
	if (hCurSel!=NodeHandle_Null)
	{
		CGuiPanel_Entity* panel=((CGuiPanel_Entity*)GetParent());
		panel->OnPrlTreeDblClk(0,0);
	}
...
NodeHandle CProtoThumbnailsList::_GetCurSel()
{
	...
	if (nSelect==1)
	{
		...
		if (idxItem!=-1)
			return (NodeHandle)_handlesItem[idxItem];
	}
...
void CProtoThumbnailsList::_SetCurSel(NodeHandle h)
{
	...
	for (int i=0;i<c;i++)
	{
		if (h==(NodeHandle)_handlesItem[i])
		{
		...
		if (lib)
		{
			...
			if (hSel!=_hSel)
			{
				...
				if (ntree->GetType(hSel)==NODETYPE_FOLDER)
					handles=ntree->Enum(hSel,NODETYPE_PROTO,c);
				for (int i=0;i<c;i++)
				{
					...
					if (img.Load(fromMBCS(path.c_str())))
					{
						...
						int idxItem = InsertItem(GetItemCount(), _T(""), _il.GetImageCount() - 1);
						if (idxItem>=0)
							_handlesItem.push_back(h);
					}
				}
				_hSel=hSel;
			}
		...
		if (propToFind)
		{
			...
			if (nmBG!=StringID_Invalid)
			{
				...
				if (_util.LoadBGPads(nmBG,pads))
				{
					...
					extern void BehaviorValueCache_PreLoad(BhvValuesCache *cache,BhvValues *values,CBehaviorGraphPads &pads,BOOL bSource,BOOL bParam);
					BehaviorValueCache_PreLoad(cache,(BhvValues *)objSub->GetOwner(),pads,FALSE,TRUE);
...
BEGIN_MESSAGE_MAP(CGuiPanel_Entity, CGuiPanel)
	...
	ON_COMMAND(IDC_FORCERELOADPROTOLIB,OnReloadProtoLib)
...
ProtoID CGuiPanel_Entity::_GetCurSelProto()
{
	...
	if(ntree)
	{
		...
		{
			if (ntree->GetType(hSel)!=NODETYPE_PROTO)
				hSel=NodeHandle_Null;
		}
		if (!hSel)
			hSel=_tbns.GetCurSel();
...
void CGuiPanel_Entity::UpdateUI()
{
	...
	if (TRUE)
	{
		...
		CString s;
		wnd->GetWindowText(s);
		if (title != toMBCS((LPCTSTR)s))
			wnd->SetWindowText(fromMBCS(title.c_str()));
	}
...
{
	if (IDOK != AfxMessageBox(_T("为地图上所有需要GUID的对象自动生成一个(如果还没有生成的话),该操作会清楚UndoBuffer,确认吗?"), MB_OKCANCEL))
		return;
	GuiData_EntityMap*dataMap=(GuiData_EntityMap*)FindData("entitymap");
	...
	if (TRUE)
	{
		...
		for (int j=rc.Top();j<rc.Bottom();j+=step)
		{
			...
			DWORD sz=mp->EnumEntities((float)center.x,(float)center.y,4.0f+(float)step,addrs.data(),addrs.size());
...
void CGuiPanel_Entity::_AdjustCtrlWidth(CWnd &wnd,int wDlg)
{
	int w;
	CRect rc;
	...
	if (wnd.GetSafeHwnd())
	{
		GET_CONTROL_RECT(this,wnd.GetDlgCtrlID(),rc);
		w=wDlg-rc.left*2;
		if (w<1)
			w=1;
		wnd.SetWindowPos(NULL,0,0,w,rc.Height(),SWP_NOMOVE|SWP_NOZORDER);
	}

}


void CGuiPanel_Entity::OnSize(UINT nType, int cx, int cy)
{
	CGuiPanel::OnSize(nType,cx,cy);


	_AdjustCtrlWidth(_tree,cx);
	_AdjustCtrlWidth(_tbns,cx);
	_AdjustCtrlWidth(_page,cx);

	if (_tbns.GetSafeHwnd())
		_tbns.ResetContent();
