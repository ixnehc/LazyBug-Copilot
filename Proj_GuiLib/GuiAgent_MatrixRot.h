...
public:
	...
	virtual BOOL OnBeginDrag(int x,int y,DWORD flag)	{return TRUE;}
...
private:
	...
	BOOL _HitCircleTest(HitProbe & probe, i_math::vector3df &center ,i_math::vector3df &normal,float radius,BOOL bUpdate,BOOL bClip = TRUE);
	BOOL _HitCenterTest(int x,int y);

	void _HitTest(int x,int y);
	
	void _GetExTestPos(i_math::vector3df * testPos,i_math::vector3df &center,ICamera *cam,float len);
	//member
protected:
	RotCtrlArg _arg;
