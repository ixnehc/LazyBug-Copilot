...
BOOL CAgent_MatrixScale::Bind(MatrixEditData data)
{
...
void CAgent_MatrixScale::OnEndDrag(int x,int y)
{
	_matScaleEdit.makeIdentity();
	_funEndEditListener(this);
}
void CAgent_MatrixScale::OnDrag(int x,int y)
{
...
{
	i_math::vector3df quad[4],zpos[3];
	DWORD flags[3];
...
{	\
	...
	DWORD flagActive;
	flagActive = flags[0];
	matView.transformVect(zpos[0],zpos[0]);
	float z = zpos[0].z;
	if(num>1)
	for(int i=1;i<num;i++)
	{
		matView.transformVect(zpos[i],zpos[i]);
		if(zpos[i].z<z)
		{
			z = zpos[i].z;
			flagActive = flags[i];
...
#define DrawAxisWire_CALSCALE(t0,t1,t2,len)\
	...
	float lenTaper;
	lenTaper = DrawDirLineTaper(rp,_center,_nx,_lenAxisX,colX,_arg.colX);
	lenTaper = DrawDirLineTaper(rp,_center,_ny,_lenAxisY,colY,_arg.colY);
	lenTaper = DrawDirLineTaper(rp,_center,_nz,_lenAxisZ,colZ,_arg.colZ);
