...
bool DetectCodeTabCount(const std::vector<LineInfo> &oldLines, int& tabCount)
{
	std::map<int, int> indentCounts; // 缩进级别 -> 出现次数
	
	for (const auto& lineInfo : oldLines)
	{
		const std::string& line = lineInfo.content;
	...
	for (int possibleTab = 2; possibleTab <= 8; possibleTab++)
	{
		int support = 0;
		for (const auto& pair : indentCounts)
		{
...
//把代码行的头部的空格转化为tab
void NormalizeLinesHead(std::vector<LineInfo>& lines)
{
	...
	{
		// 转换原始内容
		NormalizeCodeLineHead(lineInfo.content, tabCount);
		
		// 同时更新用于比较的内容
		lineInfo.contentForCompare = lineInfo.content;
		
		// 去除尾部空白字符用于比较
		size_t end = lineInfo.contentForCompare.size();
		while (end > 0 && std::isspace((unsigned char)lineInfo.contentForCompare[end - 1])) 
		{
			end--;
		}
		lineInfo.contentForCompare = lineInfo.contentForCompare.substr(0, end);
	}
}
...
void GenerateDiffString(const std::string& oldContent, const std::string& newContent, std::string& diffString)
{
	...
	// 获取差异信息
	CompareCodeStrings(oldContent, newContent, diffs);
