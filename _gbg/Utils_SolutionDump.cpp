...
// -----------------------------------------------------------------------
// 内部辅助：递归遍历一个 IVsHierarchy 节点，收集所有源文件路径
// pHier      : 当前项目层次结构
// itemid     : 当前节点 ID（从 VSITEMID_ROOT 开始）
// outFiles   : 收集到的文件路径列表
// -----------------------------------------------------------------------
static void CollectFilesFromHierarchy(IVsHierarchy* pHier,
	VSITEMID itemid,
	std::vector<ProjFile>& outFiles)
{
	if (!pHier || itemid == VSITEMID_NIL)
		return;

	// --- 跳过"External Dependencies"等系统虚拟文件夹 ---
	// VSHPROPID_IsNonMemberItem == true 表示该节点是编译器自动推断出的依赖，
	// 不是用户真正添加到项目里的文件，直接跳过整个子树。
	if (itemid != VSITEMID_ROOT)
	{
		CComVariant varNonMember;
		HRESULT hrNM = pHier->GetProperty(itemid, VSHPROPID_IsNonMemberItem, &varNonMember);
		if (SUCCEEDED(hrNM) && varNonMember.vt == VT_BOOL && varNonMember.boolVal != VARIANT_FALSE)
			return; // 整棵子树都跳过
	}

	// --- 尝试获取当前节点的磁盘路径 ---
	// GetCanonicalName 返回节点对应的完整路径（文件或目录）
	CComBSTR bstrName;
	HRESULT hr = pHier->GetCanonicalName(itemid, &bstrName);
	if (SUCCEEDED(hr) && bstrName && bstrName.Length() > 0)
	{
		std::wstring wpath(bstrName);

		// 只收集文件（不收集目录、项目节点本身等）
		// 判断依据：路径不以 '\' 或 '/' 结尾，且磁盘上确实是文件
		bool endsWithSlash = (!wpath.empty() &&
			(wpath.back() == L'\\' || wpath.back() == L'/'));

		if (!endsWithSlash)
		{
			// 先把路径转小写，用于系统目录检测
// 			std::wstring wlower = wpath;
// 			std::transform(wlower.begin(), wlower.end(), wlower.begin(), ::towlower);

			// 过滤掉 VS 安装目录 / Windows SDK / LLVM 等系统路径下的文件
//			if (!IsSystemPath(wlower))
			{
				...
				{
					ProjFile pf;
					pf.filePath = widechar_to_utf8(wpath.c_str());
					//pf.lowerCasedFilePath和pf.fileName暂时不填

// 					pf.lowerCasedFilePath = widechar_to_utf8(wlower.c_str());
// 					// 提取文件名部分
// 					auto pos = wpath.find_last_of(L"\\/");
// 					std::wstring wname = (pos != std::wstring::npos) ? wpath.substr(pos + 1) : wpath;
// 					pf.fileName = widechar_to_utf8(wname.c_str());
	...
	hr = pHier->GetProperty(itemid, VSHPROPID_FirstChild, &varChild);
	if (SUCCEEDED(hr) && varChild.vt != VT_EMPTY)
	{
		VSITEMID childId = (varChild.vt == VT_I4) ? (VSITEMID)(ULONG)varChild.lVal
			: (VSITEMID)(ULONG)varChild.uiVal;
		while (childId != VSITEMID_NIL)
		{
			CollectFilesFromHierarchy(pHier, childId, outFiles);

			// 获取下一个兄弟节点
			CComVariant varSibling;
			hr = pHier->GetProperty(childId, VSHPROPID_NextSibling, &varSibling);
			if (FAILED(hr) || varSibling.vt == VT_EMPTY)
				break;

			childId = (varSibling.vt == VT_I4) ? (VSITEMID)(ULONG)varSibling.lVal
				: (VSITEMID)(ULONG)varSibling.uiVal;
		}
	}
}
...
static void FillVcxprojSetting2(const char* dbFolder, IDispatch* pProj,
	...
	ProjSetting& setting,
	const std::vector<ProjFile>& projFiles)
{
	if (!pProj)
		return;
	...
	if (!pchHeader.empty() && pchHeader.find(L"$(") == std::wstring::npos)
	{
		...
		if (attr == INVALID_FILE_ATTRIBUTES)
		{
			...
			{
				// 从 filePath 中提取文件名（不含路径）
				std::string fileName = GetFileName(projFile.filePath);

				// 转小写用于比较
				std::string fileNameLower = fileName;
				StringLower(fileNameLower);

				if (fileNameLower == pchFileNameLowerUtf8)
				{
					// 找到同名文件，更新 pch 路径
					std::string lowerPath = projFile.filePath;
					StringLower(lowerPath);
					setting.lowerCasedPchFullPath = lowerPath;
					break;
				}
			}
		}
...
static void FillVcxprojSetting(const char* dbFolder, IDispatch* pDTEDisp,
	const std::wstring& projFileAbsPath,
	ProjSetting& setting,
	const std::vector<ProjFile>& projFiles)
{
	// 1. DTE2.Solution
	CComPtr<IDispatch> pSolution = DispGetDispProp(pDTEDisp, L"Solution");
	if (!pSolution)
		return;

	// 2. DTE2.Solution.Projects
	CComPtr<IDispatch> pProjects = DispGetDispProp(pSolution, L"Projects");
	if (!pProjects)
		return;
	...
	// 3. 在 Projects 集合里找到与 projFileAbsPath 匹配的 Project
	CComPtr<IDispatch> pTargetProject;
	std::wstring lowerTarget = projFileAbsPath;
	...
	for (long i = 1; i <= projCount; i++)
	{
		...
		std::wstring fullName = DispGetStringProp(pProj, L"FullName");
		std::transform(fullName.begin(), fullName.end(), fullName.begin(), ::towlower);

		if (fullName == lowerTarget)
		{
			pTargetProject = pProj;
			break;
		}
	}
	...
	// 4. Project.Object → VCProject (IDispatch)
	CComPtr<IDispatch> pVCProject = DispGetDispProp(pTargetProject, L"Object");
	if (!pVCProject)
		return;

	// 5. VCProject.Configurations
	CComPtr<IDispatch> pConfigs = DispGetDispProp(pVCProject, L"Configurations");
	if (!pConfigs)
		return;
	...
	// 6. VCConfiguration.Tools → 找 VCCLCompilerTool
	//    Tools 集合里按名字找：Item("VCCLCompilerTool")
	CComPtr<IDispatch> pTools = DispGetDispProp(pConfig, L"Tools");
	if (!pTools)
		return;
	...
	// 7. 读取 AdditionalIncludeDirectories（已展开宏后的字符串）
	//    VCCLCompilerTool 有两个属性：
	//      AdditionalIncludeDirectories  → 原始字符串（含宏）
	//      FullIncludePath               → 展开宏后的完整路径（推荐）
	std::wstring includeRaw = DispGetStringProp(pCompilerTool, L"FullIncludePath");
...
bool Util_GenerateSolutionDump(const char *dbFolder,CComPtr<IVsSolution> pSolution, SolutionDump& dmp)
{
	if (!pSolution)
		return false;
	...
	// ── 0. 获取 DTE2 IDispatch，用于后续读取 VCProject 属性 ──────────────
	// DTE2 通过 IServiceProvider 取 SID_SDTE / IID_DTE
	...
	// ── 1. 获取 solution 文件所在目录和文件路径 ──────────────────────────
	std::wstring solutionDir;
	{
		CComBSTR bstrSolDir, bstrSolFile, bstrSolOpt;
		if (SUCCEEDED(pSolution->GetSolutionInfo(&bstrSolDir, &bstrSolFile, &bstrSolOpt))
			&& bstrSolDir && bstrSolDir.Length() > 0)
		{
			solutionDir = bstrSolDir;
		}
		if (bstrSolFile && bstrSolFile.Length() > 0)
		{
			std::wstring solFile(bstrSolFile);
			std::transform(solFile.begin(), solFile.end(), solFile.begin(), ::towlower);
			dmp.lowerCasedPath = widechar_to_utf8(solFile.c_str());
		}
	}

	// ── 2. 枚举 solution 内所有已加载项目 ────────────────────────────────
	CComPtr<IEnumHierarchies> pEnum;
	HRESULT hr = pSolution->GetProjectEnum(
	...
	while (pEnum->Next(1, &pHier, &fetched) == S_OK && fetched == 1)
	{
		SolutionDump::ProjDump projDump;

		// ── 3. 取项目文件相对路径（相对于 solution 目录） ────────────────
		std::wstring projAbsPath;
		CComBSTR bstrUniqueName;
		hr = pSolution->GetUniqueNameOfProject(pHier, &bstrUniqueName);
		if (SUCCEEDED(hr) && bstrUniqueName && bstrUniqueName.Length() > 0)
		{
			std::wstring uniqueName(bstrUniqueName);

			// GetUniqueNameOfProject 返回的是相对于 solution 的相对路径
			// 需要拼成绝对路径
			if (PathIsRelativeW(uniqueName.c_str()) && !solutionDir.empty())
			{
				wchar_t buf[MAX_PATH] = {};
				PathCombineW(buf, solutionDir.c_str(), uniqueName.c_str());
				projAbsPath = buf;
			}
			else
			{
				projAbsPath = uniqueName;
			}

			// 规范化路径
			wchar_t canonical[MAX_PATH] = {};
			if (PathCanonicalizeW(canonical, projAbsPath.c_str()))
				projAbsPath = canonical;

			// 小写存储
			std::wstring wLower = projAbsPath;
			std::transform(wLower.begin(), wLower.end(), wLower.begin(), ::towlower);
			projDump.lowerCasedPath = widechar_to_utf8(wLower.c_str());
		}

		// ── 4. 如果是 vcxproj，读取 include 路径和 pch 设置 ────────────
		bool isVcxproj = false;
		bool isNoExt = false;
		if (!projAbsPath.empty())
		{
			// 扩展名判断（大小写不敏感）
			wchar_t ext[_MAX_EXT] = {};
			_wsplitpath_s(projAbsPath.c_str(),
		...
		// ── 5. 递归收集项目内所有文件 ────────────────────────────────────
		CollectFilesFromHierarchy(pHier, VSITEMID_ROOT, projDump.files);

		// ── 6. 如果是 vcxproj，在收集文件后读取 include 路径和 pch 设置 ───
		if (isVcxproj && pDTEDisp)
		{
			FillVcxprojSetting(dbFolder, pDTEDisp, projAbsPath, projDump.setting, projDump.files);
		}
		...
		if (isValidProj)
			dmp.projs.push_back(std::move(projDump));
...
// -----------------------------------------------------------------------
static void CollectFilesFromProjectItem(IDispatch* pItem, std::vector<ProjFile>& outFiles)
{
	if (!pItem)
		return;
...
// -----------------------------------------------------------------------
static void CollectFilesFromProject(IDispatch* pProj, std::vector<ProjFile>& outFiles)
{
	if (!pProj)
		return;
...
// -----------------------------------------------------------------------
bool Util_GenerateSolutionDump2(const char* dbFolder, CComPtr<IVsSolution> pSolution, SolutionDump& dmp)
{
	if (!pSolution)
		return false;
	...
	// 获取 solution 所在目录
	std::wstring solutionDir = solutionPath;
	auto slashPos = solutionDir.find_last_of(L"\\/");
	if (slashPos != std::wstring::npos)
		solutionDir = solutionDir.substr(0, slashPos);

	// ── 3. 获取 Solution.Projects 集合并枚举所有项目（包括 Solution Folder 中的）
	CComPtr<IDispatch> pProjects = DispGetDispProp(pDTESolution, L"Projects");

	if (!pProjects)
		return false;

	// 递归枚举所有项目（包括 Solution Folder 中的项目）
	...
	for (size_t i = 0; i < allProjects.size(); i++)
	{
		...
		if (projFullPath.empty())
			continue; // 跳过没有物理路径的无效项目（如 Miscellaneous Files、Solution Items、Unloaded Projects 等）
		...
		if (isVcxproj)
		{
			// 使用 FillVcxprojSetting2，直接传入已枚举的 pProj
...
bool Util_GenerateSolutionDumpTimeStamps(SolutionDump& dmp, SolutionDumpTimeStamps& timeStamps)
{
	...
	for (const auto& proj : dmp.projs)
	{
		...
		tsProj.lowerCasedPath = proj.lowerCasedPath;
