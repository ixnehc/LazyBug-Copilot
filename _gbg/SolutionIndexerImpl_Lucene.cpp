...
bool CSolutionIndexerImpl_Lucene::Find(const char* key, int maxResult, FindInFileResults& results)
{
	...
	try
	{
		...
		// 解析查询
		Lucene::String keyW = ToLuceneString(key);
		
		Lucene::String newKeyW;
		Lucene::String currentWord;
		for (size_t i = 0; i <= keyW.length(); ++i) {
			wchar_t c = (i < keyW.length()) ? keyW[i] : 0;
			if ((c >= L'a' && c <= L'z') || (c >= L'A' && c <= L'Z') || (c >= L'0' && c <= L'9') || c == L'_') {
				currentWord += c;
			} else {
				if (!currentWord.empty()) {
					if (!newKeyW.empty()) newKeyW += L" ";
					newKeyW += L"*" + currentWord + L"*";
					currentWord.clear();
				}
				if (c > 127) {
					if (!newKeyW.empty()) newKeyW += L" ";
		...
		Lucene::QueryPtr query = parser->parse(newKeyW);
