...
{
public:
	const char** GetIncludes(int& n)
	{
		static const char* standardIncludePaths[] =
		{
			"C:\\Program Files\\Microsoft Visual Studio\\2022\\Professional\\VC\\Tools\\MSVC\\14.42.34433\\include",
			"C:\\Program Files\\Microsoft Visual Studio\\2022\\Professional\\VC\\Tools\\MSVC\\14.42.34433\\atlmfc\\include",
		...
		const int numStandardPaths = sizeof(standardIncludePaths) / sizeof(standardIncludePaths[0]);
	...
	bool IsUnderSystemDir(const char* path)
	{
		...
		for (int i = 0;i < n;i++)
		{
			if (CheckPathContaining(dirs[i], path))
				return true;
		}
		return false;
	}
...
// 	__M("Here");
	...
	int numStandardPaths;
	const char** standardIncludePaths = g_sysDirs.GetIncludes(numStandardPaths);
	std::vector<const char*> args;
	args.push_back("-x");
	args.push_back("c++");
	args.push_back("-std=c++17");
	args.push_back("-I.");
	for (int i = 0; i < numStandardPaths; i++)
	{
		args.push_back("-I");
		args.push_back(standardIncludePaths[i]);
	}
	if (request.setting)
	{
...
//	MessageBoxA(NULL, request.parseFilePath.c_str(), "Current Parsing:", MB_OK);
	...
	// 准备编译参数
	std::vector<const char*> args;
	args.reserve(4 + numStandardPaths * 2 + (request.setting ? request.setting->additionalIncludeFullPathes.size() : 0) * 2);
...
// 		__M(request.lowerCasedParseFilePath.c_str());
	...
	CXTranslationUnit unit = clang_parseTranslationUnit(
		...
		nullptr,
		0,
		isGeneratingPCH ? (CXTranslationUnit_ForSerialization | CXTranslationUnit_DetailedPreprocessingRecord) : CXTranslationUnit_None
	);
