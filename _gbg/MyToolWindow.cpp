...
void LazyBugPlugInWindowPane::UpdateSolutionDump()
{
	...
	// 获取solution文件路径
	const char* slnPathA = GetOpenedSlnPath();
	if (!slnPathA || slnPathA[0] == '\0')
		return;

	// 2. 获取solution db folder路径, 构建.slndmp和.slndmpts文件路径
	const char* dbFolderPath = GetOpenedDBFolderPath();
	if (!dbFolderPath || dbFolderPath[0] == '\0')
		return;

	std::string slnDmpPath = dbFolderPath;
	slnDmpPath += "\\.slndmp";
	std::string slnDmpTsPath = dbFolderPath;
	slnDmpTsPath += "\\.slndmpts";

	// 3. 加载已有的时间戳快照, 检查是否过期
	SolutionDumpTimeStamps savedTimeStamps;
	LoadSolutionDumpTimeStamp(slnDmpTsPath.c_str(), savedTimeStamps);

	if (savedTimeStamps.IsValid() && !Util_CheckSolutionDumpOutOfDate(savedTimeStamps))
	{
		// 时间戳快照存在且未过期, 不需要更新
		return;
	}

	// 4. 生成SolutionDump并保存，记录耗时
	SolutionDump dump;
	auto startTime = std::chrono::high_resolution_clock::now();
	bool success = Util_GenerateSolutionDump2(dbFolderPath, g_ps.pSolution, dump);
	...
	// 将耗时记录到文件
	std::string timingFilePath = dbFolderPath;
	timingFilePath += "\\.slndmp_timing";
	FILE* fp = nullptr;
	if (fopen_s(&fp, timingFilePath.c_str(), "w") == 0 && fp)
	{
		fprintf(fp, "Util_GenerateSolutionDump execution time: %lld ms\n", duration);
		fclose(fp);
	}

	SaveSolutionDump(slnDmpPath.c_str(), dump);

	// 5. 生成时间戳快照并保存
	SolutionDumpTimeStamps newTimeStamps;
	Util_GenerateSolutionDumpTimeStamps(dump, newTimeStamps);
	SaveSolutionDumpTimeStamp(slnDmpTsPath.c_str(), newTimeStamps);
}
